...
static inline bool CheckDXLevel() {
  ...
  if (mat_dxlevel->GetInt() < 90) {
    ...
    does not support. -dxlevel must be at least 90.";
    U::Core.AppendFailText(sMessage);
    SDK::Output("Amalgam", sMessage, DEFAULT_COLOR, OUTPUT_CONSOLE |
    OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG); return false;
    */

    const char *sMessage =
        "You are running with graphics options that Amalgam does not support. "
        "It is recommended for -dxlevel to be at least 90.";
    SDK::Output("Amalgam", sMessage, DEFAULT_COLOR,
...
void CCore::Load() {
  ...
  SDK::Output("Amalgam", "Loaded", DEFAULT_COLOR,
              OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
}
...
void CCore::Unload() {
  ...
  SDK::Output("Amalgam", "Unloaded", DEFAULT_COLOR,
