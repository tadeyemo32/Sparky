...
static inline bool CheckDXLevel()
{
	...
	{
		/*
		const char* sMessage = "You are running with graphics options that Spxrky does not support. -dxlevel must be at least 90.";
		U::Core.AppendFailText(sMessage);
		SDK::Output("Spxrky", sMessage, DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
		*/

		const char* sMessage = "You are running with graphics options that Spxrky does not support. It is recommended for -dxlevel to be at least 90.";
		SDK::Output("Spxrky", sMessage, DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
...
void CCore::LogFailText()
{
	try
	{
		...
		m_ssFailStream << "Logged to Spxrky\\fail_log.txt. ";
	}
	catch (...) {}
...
void CCore::Load()
{
	...
	SDK::Output("Spxrky", "Loaded", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
}
...
void CCore::Unload()
{
	...
	SDK::Output("Spxrky", "Unloaded", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_DEBUG);
}