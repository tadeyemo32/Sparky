...
void CTicks::MoveFunc(float accumulated_extra_samples, bool bFinalTick)
{
	...
	static auto CL_Move = U::Hooks.m_mHooks[XS("CL_Move")];
...
void CTicks::MoveManage()
{
	...
	static auto sv_maxusrcmdprocessticks = H::ConVars.FindVar(XS("sv_maxusrcmdprocessticks"));
...
{
	static auto sv_maxusrcmdprocessticks_holdaim = H::ConVars.FindVar(XS("sv_maxusrcmdprocessticks_holdaim"));
...
void CTicks::Draw(CTFPlayer* pLocal)
{
	...
	if (!m_bSpeedhack)
	{
		...
		H::Draw.StringOutlined(fFont, dtPos.x, dtPos.y + 2, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOP, std::vformat(XS("Ticks {} / {}"), std::make_format_args( iTicks, iMax)).c_str());
		if (m_iWait)
			H::Draw.StringOutlined(fFont, dtPos.x, dtPos.y + fFont.m_nTall + H::Draw.Scale(18, Scale_Round) + 1, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOP, XS("Not Ready"));
		...
		H::Draw.StringOutlined(fFont, dtPos.x, dtPos.y + 2, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOP, std::vformat(XS("Speedhack x{}"), std::make_format_args( Vars::Speedhack::Amount.Value)).c_str());
}