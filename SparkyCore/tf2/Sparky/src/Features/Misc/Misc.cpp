...
void CMisc::AutoStrafe(CTFPlayer* pLocal, CUserCmd* pCmd)
{
	...
	{
		static auto cl_sidespeed = H::ConVars.FindVar(XS("cl_sidespeed"));
...
void CMisc::NoisemakerSpam(CTFPlayer* pLocal)
{
	...
	KeyValues* kv = new KeyValues(XS("use_action_slot_item_server"));
	I::EngineClient->ServerCmdKeyValues(kv);
}
...
void CMisc::CheatsBypass()
{
	...
	static auto sv_cheats = H::ConVars.FindVar(XS("sv_cheats"));
...
{
	static auto cl_wpn_sway_interp = H::ConVars.FindVar(XS("cl_wpn_sway_interp"));
	static auto cl_wpn_sway_scale = H::ConVars.FindVar(XS("cl_wpn_sway_scale"));
	...
	if (!bPost)
	{
		if (Vars::AutoPeek::Enabled.Value)
		{
			...
			else
			{
				static Timer tTimer = {};
				if (tTimer.Run(0.7f))
					H::Particles.DispatchParticleEffect(XS("ping_circle"), m_vPeekReturnPos, {});
			}
		}
		else
...
int CMisc::AntiBackstab(CTFPlayer* pLocal, CUserCmd* pCmd, bool bSendPacket)
{
	...
	for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerEnemy))
	{
		...
		if (!pWeapon
			...
			&& !(G::PrimaryWeaponType == EWeaponType::MELEE && SDK::AttribHookValue(0, XS("crit_from_behind"), pWeapon) > 0)
			&& !(pWeapon->GetWeaponID() == TF_WEAPON_FLAMETHROWER && SDK::AttribHookValue(0, XS("set_flamethrower_back_crit"), pWeapon) == 1)
...
void CMisc::PingReducer()
{
	...
	static auto cl_cmdrate = H::ConVars.FindVar(XS("cl_cmdrate"));
	...
	if (m_iWishCmdrate != iTarget)
	{
		...
		{
			NET_SetConVar tConvar = { XS("cl_cmdrate"), std::to_string(m_iWishCmdrate).c_str() };
			pNetChan->SendNetMsg(tConvar);
		}
	}

	static auto sv_maxupdaterate = H::ConVars.FindVar(XS("sv_maxupdaterate")); // force highest cl_updaterate command possible
	...
	if (m_iWishUpdaterate != iTarget)
	{
		...
		if (pNetChan && I::EngineClient->IsConnected())
		{
			NET_SetConVar tConvar = { XS("cl_updaterate"), std::to_string(m_iWishUpdaterate).c_str() };
			pNetChan->SendNetMsg(tConvar);
		}
	}
}
