...
void CAutoVote::UserMessage(bf_read& msgData)
{
	...
	{
		I::ClientState->SendStringCmd(std::vformat(XS("vote {} option2"), std::make_format_args( iVoteID)).c_str());
	}
	...
	{
		I::ClientState->SendStringCmd(std::vformat(XS("vote {} option1"), std::make_format_args( iVoteID)).c_str());
	}
}