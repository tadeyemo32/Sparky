...
static std::unordered_map<uint32_t, CommandCallback> s_mCommands = {
	AddCommand(XS("setcvar"),
	{
		...
		if (!pCVar)
		{
			SDK::Output(XSFMT(XS("Could not find {}"), sCVar).c_str());
			return;
		}

		std::string sValue = "";
		for (int i = 1; i < vArgs.size(); i++)
			sValue += XSFMT(XS("{} "), vArgs[i]);
		sValue.pop_back();
		boost::replace_all(sValue, XS("\""), "");

		pCVar->SetValue(sValue.c_str());
		SDK::Output(XSFMT(XS("Set {} to {}"), sCVar, sValue).c_str());
	})
	AddCommand(XS("getcvar"),
	{
		...
		SDK::Output(XSFMT(XS("Value of {} is {}"), sCVar, pCVar->GetString()).c_str());
	})
	AddCommand(XS("queue"),
	{
