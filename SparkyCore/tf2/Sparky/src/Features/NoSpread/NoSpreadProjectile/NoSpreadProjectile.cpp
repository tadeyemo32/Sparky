...
void CNoSpreadProjectile::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	{
		SDK::RandomSeed(SDK::SeedFileLineHash(MD5_PseudoRandom(pCmd->command_number) & 0x7FFFFFFF, XS("SelectWeightedSequence"), 0));
		for (int i = 0; i < 6; ++i)
			SDK::RandomFloat();
		...
		case TF_WEAPON_PIPEBOMBLAUNCHER:
		{
			...
			if (pWeapon->GetWeaponID() != TF_WEAPON_PIPEBOMBLAUNCHER)
				flSpeed = SDK::AttribHookValue(pLocal->InCond(TF_COND_RUNE_PRECISION) ? 3000.f : SDK::AttribHookValue(1200.f, XS("mult_projectile_speed"), pWeapon), XS("mult_projectile_range"), pWeapon);
			else
			{
				...
				flSpeed = SDK::AttribHookValue(Math::RemapVal(flCharge, 0.f, SDK::AttribHookValue(4.f, XS("stickybomb_charge_rate"), pWeapon), 900.f, 2400.f), XS("mult_projectile_range"), pWeapon);
			}
