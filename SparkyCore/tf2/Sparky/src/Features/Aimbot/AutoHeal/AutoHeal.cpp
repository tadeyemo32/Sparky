...
static inline float GetDamage(CBaseEntity* pProjectile, CTFPlayer* pOwner, CTFWeaponBase* pWeapon, CTFPlayer* pTarget, float flMult, Vec3 vProjectileOrigin, int* pType = nullptr)
{
	...
	if (pWeapon)
		flReturn *= SDK::AttribHookValue(1, XS("mult_dmg"), pWeapon);
	return flReturn;
}
...
static inline float GetMult(CBaseEntity* pProjectile, CTFWeaponBase* pWeapon, CTFPlayer* pTarget)
{
	...
	case ETFClassID::CTFProjectile_SpellLightningOrb:
		if (pProjectile->As<CTFProjectile_Rocket>()->m_bCritical())
			...
			|| pTarget->InCond(TF_COND_BLASTJUMPING) && pWeapon && SDK::AttribHookValue(0, XS("mini_crit_airborne"), pWeapon) == 1)
			flReturn = 1.36f;
		break;
	case ETFClassID::CTFProjectile_EnergyBall:
...
void CAutoHeal::GetDangers(CTFPlayer* pTarget, bool bVaccinator, float& flBulletOut, float& flBlastOut, float& flFireOut)
{
	...
	for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerEnemy))
	{
		...
		case TF_WEAPON_SNIPERRIFLE_CLASSIC:
		{
			...
			bool bPiss = SDK::AttribHookValue(0, XS("jarate_duration"), pWeapon) > 0;
			...
			if (CSniperDot* pPlayerDot = GetSniperDot(pEntity))
			{
				...
				if (bClassic && flDamage != 150.f)
					flDamage = SDK::AttribHookValue(flDamage, XS("bodyshot_damage_modify"), pWeapon);
				else
					flMult = std::max(bHeadshot || bClassic ? 3.f : bPiss ? 1.36f : 1.f, flMult);
				break;
			}
			if (SDK::AttribHookValue(0, XS("sniper_only_fire_zoomed"), pWeapon) && !bZoom)
				flDamage = 0.f;
			else if (bClassic || !bZoom)
				flDamage = SDK::AttribHookValue(50.f, XS("bodyshot_damage_modify"), pWeapon);
			else
		...
		case EWeaponType::HITSCAN:
		{
			...
			flDamage *= flMult * (pWeapon ? SDK::AttribHookValue(1, XS("mult_dmg"), pWeapon) : 1);
...
#ifdef DEBUG_VACCINATOR
			SDK::Output(XS("Projectile"), XSFMT(XS("{}, {}, {}, {}"), flDamage, flDamageInLatency, flDamageDanger, flDistanceDanger).c_str(), { 255, 200, 200 }, Vars::Debug::Logging.Value);
#endif
			switch (iType)
			{
...
#ifdef DEBUG_VACCINATOR
		SDK::Output(XS("Sentry"), XSFMT(XS("{}, {}, {}, {}"), flDamage, flDamageInLatency, flDamageDanger, flDistanceDanger).c_str(), { 255, 200, 200 }, Vars::Debug::Logging.Value);
#endif
		flBulletDanger += flDamageDanger * flDistanceDanger;
	}
	...
	for (auto pEntity : H::Entities.GetGroup(EntityEnum::WorldProjectile))
	{
		...
		SDK::Output(XSFMT(XS("Projectile {}"), iType).c_str(), XSFMT(XS("{}, {}, {}"), flDamage, flDamageDanger, flDistanceDanger).c_str(), { 255, 200, 200 }, Vars::Debug::Logging.Value);
#endif
		switch (iType)
		{
...
void CAutoHeal::Event(IGameEvent* pEvent, uint32_t uHash)
{
	...
	case FNV1A::Hash32Const(XS("player_hurt")):
	{
		...
		float flMult = SDK::AttribHookValue(1, XS("mult_dmg"), pWeapon2);
		...
		case EWeaponType::PROJECTILE:
			...
			case TF_WEAPON_FLAMETHROWER:
				...
				{
					m_iDamagedType = MEDIGUN_FIRE_RESIST;
					float flBurnMult = SDK::AttribHookValue(1, XS("mult_wpn_burndmg"), pWeapon2);
		...
		m_flDamagedTime = 1.f;
#ifdef DEBUG_VACCINATOR
		SDK::Output(XS("Hurt"), XSFMT(XS("{}, {}"), m_iDamagedType, m_flDamagedDPS).c_str(), { 255, 100, 100 });
#endif
		return;
	}
...
void CAutoHeal::Draw(CTFPlayer* pLocal)
{
	...
	for (auto& [flDanger, iResist] : vResistDangers)
		H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOP, XSFMT(XS("{}: {:.3f}"), iResist == MEDIGUN_BULLET_RESIST ? XS("Bullet") : iResist == MEDIGUN_BLAST_RESIST ? XS("Blast") : XS("Fire"), flDanger).c_str());
}
#endif