...
static inline bool ShouldTarget(CBaseEntity* pProjectile, CTFPlayer* pLocal)
{
	...
	{
		if (!SDK::AttribHookValue(1, XS("mult_dmg"), pWeapon))
			return false;
	}
...
{
	auto flRadius = SDK::AttribHookValue(1, XS("deflection_size_multiplier"), pWeapon) * 128.f;
...
void CAutoAirblast::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	if (iWeaponID != TF_WEAPON_FLAMETHROWER && iWeaponID != TF_WEAPON_FLAME_BALL || SDK::AttribHookValue(0, XS("airblast_disabled"), pWeapon))
		return;

	static auto tf_flamethrower_burstammo = H::ConVars.FindVar(XS("tf_flamethrower_burstammo"));
	int iAmmoPerShot = tf_flamethrower_burstammo->GetInt() * SDK::AttribHookValue(1, XS("mult_airblast_cost"), pWeapon);
	int iAmmo = pLocal->GetAmmoCount(pWeapon->m_iPrimaryAmmoType());
	int iBuffType = SDK::AttribHookValue(0, XS("set_buff_type"), pWeapon);
	int iChargedAirblast = SDK::AttribHookValue(0, XS("set_charged_airblast"), pWeapon);
