...
bool CAutoDetonate::GetRadius(EntityEnum::EntityEnum iGroup, CBaseEntity* pProjectile, float& flRadius, CTFWeaponBase*& pWeapon)
{
	...
	if (iGroup == EntityEnum::LocalStickies)
	{
		...
		if (!pPipebomb->m_flCreationTime() || I::GlobalVars->curtime < pPipebomb->m_flCreationTime() + SDK::AttribHookValue(0.8f, XS("sticky_arm_time"), pWeapon))
			return false;
		...
		if (!pPipebomb->m_bTouched())
		{
			...
			static auto tf_sticky_radius_ramp_time = H::ConVars.FindVar(XS("tf_sticky_radius_ramp_time"));
			static auto tf_sticky_airdet_radius = H::ConVars.FindVar(XS("tf_sticky_airdet_radius"));
