...

	{
		...
		if (Vars::Aimbot::General::Target.Value & Vars::Aimbot::General::TargetEnum::Players)
		{
			...
			if (Vars::Aimbot::Hitscan::Modifiers.Value & Vars::Aimbot::Hitscan::ModifiersEnum::ExtinguishTeam &&
				!F::AimbotGlobal.FriendlyFire() && SDK::AttribHookValue(0, XS("jarate_duration"), pWeapon) > 0)
		...
		for (auto pEntity : H::Entities.GetGroup(eGroupType))
		{
			...
			if (bTeam)
			{
				...
				if (!F::AimbotGlobal.FriendlyFire() && SDK::AttribHookValue(0, XS("jarate_duration"), pWeapon) > 0)
				{
...
int CAimbotHitscan::GetHitboxPriority(int nHitbox, CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CBaseEntity* pTarget)
{
	...
	if (pTarget->IsPlayer())
	{
		...
		case TF_WEAPON_SNIPERRIFLE_CLASSIC:
		{
			...
			if (G::CanHeadshot
				|| pLocal->InCond(TF_COND_AIMING) && (pSniperRifle->GetRifleType() == RIFLE_JARATE && SDK::AttribHookValue(0, XS("jarate_duration"), pWeapon) > 0 || Vars::Aimbot::Hitscan::Modifiers.Value & Vars::Aimbot::Hitscan::ModifiersEnum::WaitForHeadshot))
				bHeadshot = true;
			break;
		}
		case TF_WEAPON_REVOLVER:
		{
			if (SDK::AttribHookValue(0, XS("set_weapon_mode"), pWeapon) == 1 && (pWeapon->AmbassadorCanHeadshot() || Vars::Aimbot::Hitscan::Modifiers.Value & Vars::Aimbot::Hitscan::ModifiersEnum::WaitForHeadshot))
				bHeadshot = true;
		}
		}
		...
		if (Vars::Aimbot::Hitscan::Hitboxes.Value & Vars::Aimbot::Hitscan::HitboxesEnum::BodyaimIfLethal && bHeadshot)
		{
			...
			case TF_WEAPON_REVOLVER:
			{
				if (SDK::AttribHookValue(0, XS("set_weapon_mode"), pWeapon) == 1)
				{
					float flDistTo = pTarget->m_vecOrigin().DistTo(pLocal->m_vecOrigin());

					float flMult = SDK::AttribHookValue(1, XS("mult_dmg"), pWeapon);
...
bool CAimbotHitscan::ShouldFire(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd, const Target_t& tTarget)
{
		...
		&& tTarget.m_pEntity->IsPlayer())
	{
		...
		case TF_WEAPON_SNIPERRIFLE_CLASSIC:
			...
			break;
		case TF_WEAPON_REVOLVER:
			if (SDK::AttribHookValue(0, XS("set_weapon_mode"), pWeapon) == 1 && !pWeapon->AmbassadorCanHeadshot())
				return false;
		}
	}
...
void CAimbotHitscan::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	case TF_WEAPON_SNIPERRIFLE_DECAP:
	{
		...
		if (Vars::Aimbot::Hitscan::Modifiers.Value & Vars::Aimbot::Hitscan::ModifiersEnum::AutoScope && !bScoped)
		{
			...
			return;
		else if (!bScoped && SDK::AttribHookValue(0, XS("sniper_only_fire_zoomed"), pWeapon))
			return;
		break;
	}
	case TF_WEAPON_SNIPERRIFLE_CLASSIC:
