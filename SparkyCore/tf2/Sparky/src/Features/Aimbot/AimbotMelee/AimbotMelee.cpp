...
static inline std::vector<Target_t> GetTargets(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	if (Vars::Aimbot::General::Target.Value & Vars::Aimbot::General::TargetEnum::Players)
	{
		...
		if (Vars::Aimbot::Melee::WhipTeam.Value &&
			!F::AimbotGlobal.FriendlyFire() && SDK::AttribHookValue(0, XS("speed_buff_ally"), pWeapon) > 0)
...

	{
		...
		bool bWrench = pWeapon->GetWeaponID() == TF_WEAPON_WRENCH, bSapper = SDK::AttribHookValue(0, XS("set_dmg_apply_to_sapper"), pWeapon);
...
bool CAimbotMelee::CanBackstab(CBaseEntity* pTarget, CTFPlayer* pLocal, Vec3 vEyeAngles)
{
	...
	if (Vars::Aimbot::Melee::IgnoreRazorback.Value)
	{
		...
		int iBackstabShield = SDK::AttribHookValue(0, XS("set_blockbackstab_once"), pTarget, &itemList);
...
int CAimbotMelee::CanHit(Target_t& tTarget, CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	float flRange = SDK::AttribHookValue(m_flRange, XS("melee_range_multiplier"), pWeapon);
	float flHull = SDK::AttribHookValue(18, XS("melee_bounds_multiplier"), pWeapon);
...
bool CAimbotMelee::FindNearestBuildPoint(CBaseObject* pBuilding, CTFPlayer* pLocal, Vec3& vPoint)
{
	...
	m_vEyePos = pLocal->GetShootPos();
	static auto tf_obj_max_attach_dist = H::ConVars.FindVar(XS("tf_obj_max_attach_dist"));
