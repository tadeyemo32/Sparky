...
static inline std::vector<Target_t> GetTargets(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	if (Vars::Aimbot::General::Target.Value & Vars::Aimbot::General::TargetEnum::Stickies)
	{
		...
		case TF_WEAPON_PIPEBOMBLAUNCHER:
			if (SDK::AttribHookValue(0, XS("stickies_detonate_stickies"), pWeapon) == 1)
				bShouldAim = true;
			break;
...
static inline float ArmTime(CTFWeaponBase* pWeapon)
{
	...
	{
		static auto tf_grenadelauncher_livetime = H::ConVars.FindVar(XS("tf_grenadelauncher_livetime"));
		const float flLiveTime = tf_grenadelauncher_livetime->GetFloat();
		return SDK::AttribHookValue(flLiveTime, XS("sticky_arm_time"), pWeapon);
	}
...
#ifdef SPLASH_DEBUG5
				s_mTraceCount[XS("Splash out rocket check")]++;
#endif
#ifdef SPLASH_DEBUG2
	...
	if (iType & PointTypeEnum::In)
	{
...
#ifdef SPLASH_DEBUG5
			s_mTraceCount[XS("Splash in check")]++;
#endif
		...
		else // currently experimental, there may be a more efficient way to do this?
		{
			...
			if (checkPoint(trace, bErase, bNormal))
			{
				SDK::Trace(trace.endpos + trace.plane.normal, vTargetEye, MASK_SHOT, &filter, &trace);
#ifdef SPLASH_DEBUG5
				s_mTraceCount[XS("Splash in rocket check")]++;
#endif
#ifdef SPLASH_DEBUG2
...
std::vector<Point_t> CAimbotProjectile::GetSplashPoints(Vec3 vOrigin, std::vector<std::pair<Vec3, int>>& vSpherePoints, int iSimTime)
{
	...
	auto checkPoint = [&](CGameTrace& trace, bool& bErase, bool& bNormal)
		{
			...
			{
				Vec3 vForward = (m_tInfo.m_vLocalEye - trace.endpos); // optimization: removed .Normalized() as we only need the sign of dot product
	...
	for (auto it = vPoints.begin(); it != vPoints.end();)
	{
		...
		if (bValid)
		{
			...
			bValid = tPoint.m_vPoint.DistToSqr(vPos) < powf(m_tInfo.m_flRadius, 2);
		}
	...
	auto checkPoint = [&](CGameTrace& trace, bool& bErase, bool& bNormal)
		{
			...
			if (!m_tInfo.m_flGravity)
			{
				Vec3 vForward = (m_tInfo.m_vLocalEye - trace.endpos); // optimization: removed .Normalized()
				bNormal = vForward.Dot(trace.plane.normal) <= 0;
			}
			if (!bNormal)
			{
	...
	if (iFlags & ProjSimEnum::Redirect)
	{
		if (Vars::Visuals::Trajectory::Override.Value)
		{
			if (!Vars::Visuals::Trajectory::Pipes.Value)
				s_mTraceCount[XS("Setup trace calculate")]++;
		}
		else
		{
...
bool CAimbotProjectile::TestAngle(const Vec3& vPoint, const Vec3& vAngles, int iSimTime, bool bSplash, bool bSecondTest)
{
	...
	{
		if (!Vars::Visuals::Trajectory::Pipes.Value)
			s_mTraceCount[XS("Setup trace test")]++;
	}
	else
	{
		switch (pWeapon->GetWeaponID())
		...
		case TF_WEAPON_SYRINGEGUN_MEDIC:
		case TF_WEAPON_FLAME_BALL:
			s_mTraceCount[XS("Setup trace")]++;
		}
	}
	s_mTraceCount[XS("Trace init check")]++;
#endif
	m_tProjInfo = {};
	...
	for (int n = 1; n <= iSimTime; n++)
	{
		...
		{
			bool bTime = bSplash
				? trace.endpos.DistToSqr(vPoint) < powf(m_tProjInfo.m_flVelocity * TICK_INTERVAL + m_tProjInfo.m_vHull.z, 2)
...
#ifdef SPLASH_DEBUG5
				s_mTraceCount[XS("Splash vispos")]++;
#endif
#ifdef SPLASH_DEBUG4
...
void CAimbotProjectile::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	if (Vars::Aimbot::General::AimType.Value && !s_mTraceCount.empty())
	{
		...
		for (auto& [_, iTraces] : s_mTraceCount)
			iTraceCount += iTraces;
		SDK::Output(XS("Traces"), XSFMT(XS("{}"), iTraceCount).c_str());
		for (auto& [sType, iTraces] : s_mTraceCount)
			SDK::Output(XS("Traces"), XSFMT(XS("{}: {}"), sType, iTraces).c_str());
	}
	s_mTraceCount.clear();
#endif
	...
	if (pWeapon->GetWeaponID() == TF_WEAPON_PIPEBOMBLAUNCHER)
	{
		...
		flAmount = Math::RemapVal(flCharge, 0.f, SDK::AttribHookValue(4.f, XS("stickybomb_charge_rate"), pWeapon), 0.f, 1.f);
	}
	else if (pWeapon->GetWeaponID() == TF_WEAPON_CANNON)
	{
		const float flMortar = SDK::AttribHookValue(0.f, XS("grenade_launcher_mortar_mode"), pWeapon);
