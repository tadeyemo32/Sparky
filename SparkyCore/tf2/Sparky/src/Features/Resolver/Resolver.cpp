	...
	if (m_iWaitingForTarget != -1 && m_flWaitingForDamage < I::GlobalVars->curtime)
	{
		if (auto pTarget = I::ClientEntityList->GetClientEntity(I::EngineClient->GetPlayerForUserID(m_iWaitingForTarget))->As<CTFPlayer>())
		{
				...
				&& (!bAutoYaw || sign(flYaw) != sign(flYaw + Vars::Resolver::AutoResolveYawAmount.Value) && sign(flYaw) != 0))
			{
				...
				F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(m_iWaitingForTarget), XS("Cycling"), XS("pitch"), flPitch);
			}
			...
			if (bAutoYaw)
			{
				...
				F::Backtrack.ResolverUpdate(pTarget);
				F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(m_iWaitingForTarget), XS("Cycling"), XS("yaw"), flYaw);
			}
	...
	if (Vars::Resolver::CycleYaw.Value)
	{
		if (SDK::PlatFloatTime() > m_flLastYawCycle + 0.5f)
		{
			if (auto pTarget = getPlayerClosestToFOV())
			{
				...
				F::Backtrack.ResolverUpdate(pTarget);
				F::Output.ReportResolver(pTarget->entindex(), XS("Cycling"), XS("yaw"), flYaw);
			}
			...
			if (auto pTarget = getPlayerClosestToFOV())
			{
				...
				if (!m_mSniperDots.contains(iUserID))
				{
					if (fabsf(pTarget->m_angEyeAnglesX()) != 90.f)
						F::Output.ReportResolver(XS("Target not using out of bounds pitch"));
					
					float& flPitch = tData.m_flPitch;
					flPitch = Math::ClampNormalizeAngle(flPitch + Vars::Resolver::CyclePitch.Value, 90.f);

					F::Backtrack.ResolverUpdate(pTarget);
					F::Output.ReportResolver(pTarget->entindex(), XS("Cycling"), XS("pitch"), flPitch);
				}
				else
					F::Output.ReportResolver(XS("Target pitch overridden by sniper dot"));
			}
	...
	if (Vars::Resolver::CycleMinwalk.Value)
	{
		if (SDK::PlatFloatTime() > m_flLastMinwalkCycle + 0.5f)
		{
			if (auto pTarget = getPlayerClosestToFOV())
			{
				...
				F::Backtrack.ResolverUpdate(pTarget);
				F::Output.ReportResolver(pTarget->entindex(), XS("Cycling"), XS("minwalk"), bMinwalk);
			}
	...
	if (Vars::Resolver::CycleView.Value)
	{
		if (SDK::PlatFloatTime() > m_flLastViewCycle + 0.5f)
		{
			if (auto pTarget = getPlayerClosestToFOV())
			{
				...
				F::Backtrack.ResolverUpdate(pTarget);
				F::Output.ReportResolver(pTarget->entindex(), XS("Cycling"), XS("view"), bView ? XS("view to local") : XS("static"));
			}
...
void CResolver::PlayerHurt(IGameEvent* pEvent)
{
	if (m_iWaitingForTarget == -1
		|| I::EngineClient->GetPlayerForUserID(pEvent->GetInt(XS("attacker"))) != I::EngineClient->GetLocalPlayer()
		|| pEvent->GetInt(XS("userid")) != m_iWaitingForTarget)
		return;

	if (m_bWaitingForHeadshot && !pEvent->GetBool(XS("crit")))
		return;
...
void CResolver::SetYaw(int iUserID, float flValue, bool bAuto)
{
	...
	if (bAuto)
	{
		...
		F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("yaw"), XS("auto"));
	}
	else
	{
		...
		F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("yaw"), flValue);
	}
...
void CResolver::SetPitch(int iUserID, float flValue, bool bInverse, bool bAuto)
{
	...
	if (bAuto)
	{
		...
		F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("pitch"), XS("auto"));
	}
	else if (bInverse)
	{
		...
		F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("pitch"), XS("inverse"));
	}
	else
	{
		...
		F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("pitch"), flValue);
	}
...
void CResolver::SetMinwalk(int iUserID, bool bValue)
{
	...
	F::Backtrack.ResolverUpdate(I::ClientEntityList->GetClientEntity(I::EngineClient->GetPlayerForUserID(iUserID))->As<CTFPlayer>());
	F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("minwalk"), bValue);
}
...
void CResolver::SetView(int iUserID, bool bValue)
{
	...
	F::Backtrack.ResolverUpdate(I::ClientEntityList->GetClientEntity(I::EngineClient->GetPlayerForUserID(iUserID))->As<CTFPlayer>());
	F::Output.ReportResolver(I::EngineClient->GetPlayerForUserID(iUserID), XS("Set"), XS("view"), bValue ? XS("view to local") : XS("static"));
}
