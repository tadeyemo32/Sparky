...
static inline void StorePlayer(CTFPlayer* pPlayer, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, PlayerCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & ESPEnum::Distance && !bLocal)
	{
		...
		tCache.m_vText.emplace_back(ALIGN_BOTTOM, XSFMT(XS("[{:.0f}M]"), vDelta.Length2D() / 41), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
	}
				...
				if (auto iParty = H::Entities.GetParty(uAccountID))
				{
					...
					if (int iPartyCount = H::Entities.GetPartyCount() + 1; pTag->m_bLabel)
					{
						if (!--iParty)
							...
							vTags.emplace_back(XSFMT(XS("{}: {}"), pTag->m_sName, iParty), pTag->m_tColor.HueShift(iParty * 360.f / iPartyCount), pTag->m_iPriority);
					}
				}
	...
	if (pGroup->m_iESP & ESPEnum::HealthText)
		tCache.m_vText.emplace_back(ALIGN_LEFT, XSFMT(XS("{}"), flHealth), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
	...
	if (pGroup->m_iESP & (ESPEnum::UberBar | ESPEnum::UberText) && iClassNum == TF_CLASS_MEDIC)
	{
		...
		if (pMediGun && pMediGun->GetClassID() == ETFClassID::CWeaponMedigun)
		{
			...
			if (pGroup->m_iESP & ESPEnum::UberText)
				tCache.m_vText.emplace_back(ALIGN_BOTTOMRIGHT, XSFMT(XS("{:.0f}%"), flUber * 100), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::Ping && pResource && !bLocal)
	{
		...
		if (iPing && (iPing >= 200 || iPing <= 5))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("{}MS"), iPing), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
	}
	...
	if (pGroup->m_iESP & ESPEnum::KDR && pResource && !bLocal)
	{
		...
		if (iKills >= 20)
		{
			...
			if (iKDR >= 10)
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("High KD [{} / {}]"), iKills, iDeaths), Vars::Colors::IndicatorTextMid.Value, Vars::Menu::Theme::Background.Value);
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::Flags)
	{
		...
		if (float flInvis = pPlayer->GetEffectiveInvisibilityLevel())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Invis {:.0f}%"), flInvis * 100), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		...
		if (pPlayer->InCond(TF_COND_AIMING) || pPlayer->InCond(TF_COND_ZOOMED))
		{
			...
			case TF_WEAPON_SNIPERRIFLE_DECAP:
			{
				if (bLocal)
				{
					tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Charging {:.0f}%"), Math::RemapVal(pWeapon->As<CTFSniperRifle>()->m_flChargedDamage(), 0.f, 150.f, 0.f, 100.f)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
					break;
				}
				else
				{
					...
					if (CSniperDot* pPlayerDot = GetSniperDot(pPlayer))
					{
						...
						tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Charging {:.0f}%"), Math::RemapVal(TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick) - pPlayerDot->m_flChargeStartTime() - 0.3f, 0.f, flChargeTime, 0.f, 100.f)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
						break;
					}
				}
			...
			case TF_WEAPON_COMPOUND_BOW:
				if (bLocal)
				{
					tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Charging {:.0f}%"), Math::RemapVal(TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick) - pWeapon->As<CTFPipebombLauncher>()->m_flChargeBeginTime(), 0.f, 1.f, 0.f, 100.f)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
					break;
				}
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Charging"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
				break;
		...
		if (Vars::Debug::Info.Value && !pPlayer->IsDormant() && !bLocal)
		{
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Lag {}, {}"), iAverage, iCurrent), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		}
	}
}
...
static inline void StoreBuilding(CBaseObject* pBuilding, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, BuildingCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & (ESPEnum::AmmoBars | ESPEnum::AmmoText) && pBuilding->IsSentrygun() && !pBuilding->m_bBuilding())
	{
		...
		{
			tCache.m_vText.emplace_back(ALIGN_BOTTOMRIGHT, XSFMT(XS("{}"), iShells), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
			if (iMaxRockets)
				tCache.m_vText.back().m_sText += XSFMT(XS(", {}"), iRockets);
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::Level && !bIsMini)
		tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Level {}"), pBuilding->m_iUpgradeLevel()), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

	if (pGroup->m_iESP & ESPEnum::Flags)
	{
		if (!pBuilding->IsDormant() && pBuilding->m_bBuilding())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("{:.0f}%"), pBuilding->m_flPercentageConstructed() * 100), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
...
static inline void StoreObjective(CBaseEntity* pObjective, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, EntityCache_t>& mCache)
{
	...
	case ETFClassID::CCaptureFlag:
	{
		...
		if (pGroup->m_iESP & ESPEnum::IntelReturnTime && pIntel->m_nFlagStatus() == TF_FLAGINFO_DROPPED)
		{
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XSFMT(XS("Return {:.1f}s"), pIntel->m_flResetTime() - TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick)).c_str(), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		}
