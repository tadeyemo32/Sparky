...
MAKE_SIGNATURE(CTFPlayerSharedUtils_GetEconItemViewByLoadoutSlot, XS("client.dll"), XS("48 89 6C 24 ? 56 41 54 41 55 41 56 41 57 48 83 EC"), 0x0);
MAKE_SIGNATURE(CEconItemView_GetItemName, XS("client.dll"), XS("40 53 48 83 EC ? 48 8B D9 C6 81 ? ? ? ? ? E8 ? ? ? ? 48 8B 8B"), 0x0);
...
static inline void StorePlayer(CTFPlayer* pPlayer, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, PlayerCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & ESPEnum::Distance && !bLocal)
	{
		...
		tCache.m_vText.emplace_back(ALIGN_BOTTOM, std::vformat(XS("[{:.0f}M]"), std::make_format_args( vDelta.Length2D() / 41)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
	}
				...
				if (auto iParty = H::Entities.GetParty(uAccountID))
				{
					...
					if (int iPartyCount = H::Entities.GetPartyCount() + 1; pTag->m_bLabel)
					{
						if (!--iParty)
							...
							vTags.emplace_back(std::vformat(XS("{}: {}"), std::make_format_args( pTag->m_sName, iParty)), pTag->m_tColor.HueShift(iParty * 360.f / iPartyCount), pTag->m_iPriority);
					}
				}
	...
	if (pGroup->m_iESP & ESPEnum::HealthText)
		tCache.m_vText.emplace_back(ALIGN_LEFT, std::vformat(XS("{}"), std::make_format_args( flHealth)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
	...
	if (pGroup->m_iESP & (ESPEnum::UberBar | ESPEnum::UberText) && iClassNum == TF_CLASS_MEDIC)
	{
		...
		if (pMediGun && pMediGun->GetClassID() == ETFClassID::CWeaponMedigun)
		{
			...
			if (pGroup->m_iESP & ESPEnum::UberText)
				tCache.m_vText.emplace_back(ALIGN_BOTTOMRIGHT, std::vformat(XS("{:.0f}%"), std::make_format_args( flUber * 100)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::LagCompensation && !pPlayer->IsDormant() && !bLocal)
	{
		if (H::Entities.GetLagCompensation(iIndex))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Lagcomp"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
	}
	...
	if (pGroup->m_iESP & ESPEnum::Ping && pResource && !bLocal)
	{
		...
		if (iPing && (iPing >= 200 || iPing <= 5))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("{}MS"), std::make_format_args( iPing)), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
	}
	...
	if (pGroup->m_iESP & ESPEnum::KDR && pResource && !bLocal)
	{
		...
		if (iKills >= 20)
		{
			...
			if (iKDR >= 10)
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("High KD [{} / {}]"), std::make_format_args( iKills, iDeaths)), Vars::Colors::IndicatorTextMid.Value, Vars::Menu::Theme::Background.Value);
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::Buffs)
	{
		if (pPlayer->InCond(TF_COND_INVULNERABLE) ||
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Uber"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_MEGAHEAL))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Megaheal"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_PHASE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Bonk"), Vars::Colors::IndicatorTextMid.Value, Vars::Menu::Theme::Background.Value);
		...
		if (pWeapon)
		{
			if (bMiniCrits && SDK::AttribHookValue(0, XS("minicrits_become_crits"), pWeapon)
				|| SDK::AttribHookValue(0, XS("crit_while_airborne"), pWeapon) && pPlayer->InCond(TF_COND_BLASTJUMPING))
		...
		if (bCrits)
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Crits"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		else if (bMiniCrits)
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Mini-crits"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);

		/* vaccinator effects */
		if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) || pPlayer->InCond(TF_COND_BULLET_IMMUNE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Bullet+"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_MEDIGUN_SMALL_BULLET_RESIST))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Bullet"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_BLAST_RESIST) || pPlayer->InCond(TF_COND_BLAST_IMMUNE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Blast+"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_MEDIGUN_SMALL_BLAST_RESIST))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Blast"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_MEDIGUN_UBER_FIRE_RESIST) || pPlayer->InCond(TF_COND_FIRE_IMMUNE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Fire+"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_MEDIGUN_SMALL_FIRE_RESIST))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Fire"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_OFFENSEBUFF))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Banner"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_DEFENSEBUFF))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Battalions"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_REGENONDAMAGEBUFF))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Conch"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_RUNE_STRENGTH))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Strength"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_HASTE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Haste"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_REGEN))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Regen"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_RESIST))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Resistance"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_VAMPIRE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Vampire"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_REFLECT))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Reflect"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_PRECISION))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Precision"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_AGILITY))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Agility"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_KNOCKOUT))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Knockout"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_KING))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("King"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_PLAGUE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Plague"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_RUNE_SUPERNOVA))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Supernova"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_POWERUPMODE_DOMINANT))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Dominant"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		...
		for (int i = 0; i < MAX_WEAPONS; i++)
		{
			...
			{
			case 0: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Fireball"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 1: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Bats"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 2: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Heal"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 3: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Pumpkins"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 4: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Jump"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 5: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Stealth"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 6: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Teleport"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 7: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Lightning"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 8: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Minify"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 9: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Meteors"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 10: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Monoculus"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 11: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Skeletons"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 12: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Glove"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 13: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Parachute"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 14: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Heal"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			case 15: tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Bomb"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value); break;
			}
		}
		...
		if (pPlayer->InCond(TF_COND_RADIUSHEAL) ||
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Heal"), Vars::Colors::IndicatorTextGood.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_HEALTH_OVERHEALED))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("HP+"), Vars::Colors::IndicatorTextGood.Value, Vars::Menu::Theme::Background.Value);
	...
	if (pGroup->m_iESP & ESPEnum::Debuffs)
	{
		if (pPlayer->InCond(TF_COND_MARKEDFORDEATH)
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Marked"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_URINE))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Jarate"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_MAD_MILK))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Milk"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_STUNNED))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Stun"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_BURNING))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Burn"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_BLEEDING))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Bleed"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
	}
	...
	if (pGroup->m_iESP & ESPEnum::Flags)
	{
		if (pPlayer->m_bFeignDeathReady())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("DR"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		else if (pPlayer->InCond(TF_COND_FEIGN_DEATH))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Feign"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (float flInvis = pPlayer->GetEffectiveInvisibilityLevel())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Invis {:.0f}%"), std::make_format_args( flInvis * 100)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pPlayer->InCond(TF_COND_DISGUISED))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Disguise"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		...
		if (pPlayer->InCond(TF_COND_AIMING) || pPlayer->InCond(TF_COND_ZOOMED))
		{
			...
			case TF_WEAPON_MINIGUN:
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Rev"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
				break;
			...
			case TF_WEAPON_SNIPERRIFLE_DECAP:
			{
				if (bLocal)
				{
					tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Charging {:.0f}%"), std::make_format_args( Math::RemapVal(pWeapon->As<CTFSniperRifle>()->m_flChargedDamage(), 0.f, 150.f, 0.f, 100.f))), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
					break;
				}
				else
				{
					...
					{
						float flChargeTime = std::max(SDK::AttribHookValue(3.f, XS("mult_sniper_charge_per_sec"), pWeapon), 1.5f);
						tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Charging {:.0f}%"), std::make_format_args( Math::RemapVal(TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick) - pPlayerDot->m_flChargeStartTime() - 0.3f, 0.f, flChargeTime, 0.f, 100.f))), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
						break;
					}
				}
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Charging"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
				break;
			}
			case TF_WEAPON_COMPOUND_BOW:
				if (bLocal)
				{
					tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Charging {:.0f}%"), std::make_format_args( Math::RemapVal(TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick) - pWeapon->As<CTFPipebombLauncher>()->m_flChargeBeginTime(), 0.f, 1.f, 0.f, 100.f))), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
					break;
				}
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Charging"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
				break;
		...
		if (Vars::Visuals::Removals::Taunts.Value && pPlayer->InCond(TF_COND_TAUNTING))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Taunt"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		...
		if (Vars::Debug::Info.Value && !pPlayer->IsDormant() && !bLocal)
		{
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Lag {}, {}"), std::make_format_args( iAverage, iCurrent)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		}
	}
}
...
static inline void StoreBuilding(CBaseObject* pBuilding, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, BuildingCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & ESPEnum::Name)
	{
		const char* sName = XS("Building");
		switch (pBuilding->GetClassID())
		{
		case ETFClassID::CObjectSentrygun: sName = bIsMini ? XS("Mini-Sentry") : XS("Sentry"); break;
		case ETFClassID::CObjectDispenser: sName = XS("Dispenser"); break;
		case ETFClassID::CObjectTeleporter: sName = pBuilding->m_iObjectMode() ? XS("Teleporter Exit") : XS("Teleporter Entrance");
		}
	...
	if (pGroup->m_iESP & (ESPEnum::AmmoBars | ESPEnum::AmmoText) && pBuilding->IsSentrygun() && !pBuilding->m_bBuilding())
	{
		...
		{
			tCache.m_vText.emplace_back(ALIGN_BOTTOMRIGHT, std::vformat(XS("{}"), std::make_format_args( iShells)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
			if (iMaxRockets)
				tCache.m_vText.back().m_sText += std::vformat(XS(", {}"), std::make_format_args( iRockets));
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::Level && !bIsMini)
		tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Level {}"), std::make_format_args( pBuilding->m_iUpgradeLevel())), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

	if (pGroup->m_iESP & ESPEnum::Flags)
	{
		if (!pBuilding->IsDormant() && pBuilding->m_bBuilding())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("{:.0f}%"), std::make_format_args( pBuilding->m_flPercentageConstructed() * 100)), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);

		if (pBuilding->IsSentrygun() && pBuilding->As<CObjectSentrygun>()->m_bPlayerControlled())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Wrangled"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);

		if (pBuilding->m_bHasSapper())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Sapped"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		else if (pBuilding->m_bDisabled())
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Disabled"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
	}
}

static inline const char* GetProjectileName(CBaseEntity* pProjectile)
{
	const char* sReturn = XS("Projectile");
	switch (pProjectile->GetClassID())
	{
	case ETFClassID::CTFWeaponBaseMerasmusGrenade: sReturn = XS("Bomb"); break;
	case ETFClassID::CTFGrenadePipebombProjectile: sReturn = pProjectile->As<CTFGrenadePipebombProjectile>()->HasStickyEffects() ? XS("Sticky") : XS("Pipe"); break;
	case ETFClassID::CTFStunBall: sReturn = XS("Baseball"); break;
	case ETFClassID::CTFBall_Ornament: sReturn = XS("Bauble"); break;
	case ETFClassID::CTFProjectile_Jar: sReturn = XS("Jarate"); break;
	case ETFClassID::CTFProjectile_Cleaver: sReturn = XS("Cleaver"); break;
	case ETFClassID::CTFProjectile_JarGas: sReturn = XS("Gas"); break;
	case ETFClassID::CTFProjectile_JarMilk:
	case ETFClassID::CTFProjectile_ThrowableBreadMonster: sReturn = XS("Milk"); break;
	case ETFClassID::CTFProjectile_SpellBats:
	case ETFClassID::CTFProjectile_SpellKartBats: sReturn = XS("Bats"); break;
	case ETFClassID::CTFProjectile_SpellMeteorShower: sReturn = XS("Meteors"); break;
	case ETFClassID::CTFProjectile_SpellMirv:
	case ETFClassID::CTFProjectile_SpellPumpkin: sReturn = XS("Pumpkin"); break;
	case ETFClassID::CTFProjectile_SpellSpawnBoss: sReturn = XS("Monoculus"); break;
	case ETFClassID::CTFProjectile_SpellSpawnHorde:
	case ETFClassID::CTFProjectile_SpellSpawnZombie: sReturn = XS("Skeleton"); break;
	case ETFClassID::CTFProjectile_SpellTransposeTeleport: sReturn = XS("Teleport"); break;
	case ETFClassID::CTFProjectile_Arrow: sReturn = pProjectile->As<CTFProjectile_Arrow>()->m_iProjectileType() == TF_PROJECTILE_BUILDING_REPAIR_BOLT ? XS("Repair") : XS("Arrow"); break;
	case ETFClassID::CTFProjectile_GrapplingHook: sReturn = XS("Grapple"); break;
	case ETFClassID::CTFProjectile_HealingBolt: sReturn = XS("Heal"); break;
	case ETFClassID::CTFProjectile_Rocket:
	case ETFClassID::CTFProjectile_EnergyBall:
	case ETFClassID::CTFProjectile_SentryRocket: sReturn = XS("Rocket"); break;
	case ETFClassID::CTFProjectile_BallOfFire: sReturn = XS("Fire"); break;
	case ETFClassID::CTFProjectile_MechanicalArmOrb: sReturn = XS("Short circuit"); break;
	case ETFClassID::CTFProjectile_SpellFireball: sReturn = XS("Fireball"); break;
	case ETFClassID::CTFProjectile_SpellLightningOrb: sReturn = XS("Lightning"); break;
	case ETFClassID::CTFProjectile_SpellKartOrb: sReturn = XS("Fist"); break;
	case ETFClassID::CTFProjectile_Flare: sReturn = XS("Flare"); break;
	case ETFClassID::CTFProjectile_EnergyRing: sReturn = XS("Energy"); break;
	}
	return sReturn;
}
	...
	if (pGroup->m_iESP & ESPEnum::Flags)
	{
		...
		case ETFClassID::CTFProjectile_ThrowableRepel:
			if (pProjectile->As<CTFWeaponBaseGrenadeProj>()->m_bCritical())
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Crit"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			if (pProjectile->As<CTFWeaponBaseGrenadeProj>()->m_iDeflected() && (pProjectile->GetClassID() != ETFClassID::CTFGrenadePipebombProjectile || !pProjectile->GetAbsVelocity().IsZero()))
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Reflected"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			break;
		...
		case ETFClassID::CTFProjectile_HealingBolt:
			...
			if (pProjectile->As<CTFProjectile_Arrow>()->m_bArrowAlight())
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Alight"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			break;
		case ETFClassID::CTFProjectile_Rocket:
		...
		case ETFClassID::CTFProjectile_EnergyBall:
			if (pProjectile->As<CTFProjectile_EnergyBall>()->m_bChargedShot())
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Charge"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			if (pProjectile->As<CTFBaseRocket>()->m_iDeflected())
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Reflected"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			break;
...
static inline void StoreObjective(CBaseEntity* pObjective, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, EntityCache_t>& mCache)
{
	...
	case ETFClassID::CCaptureFlag:
	{
		...
		if (pGroup->m_iESP & ESPEnum::Name)
			tCache.m_vText.emplace_back(ALIGN_TOP, XS("Intel"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		...
		if (pGroup->m_iESP & ESPEnum::Flags)
		{
			...
			case TF_FLAGINFO_HOME:
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Home"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
				break;
			case TF_FLAGINFO_DROPPED:
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Dropped"), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
				break;
			default:
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, XS("Stolen"), Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
			}
		}
		...
		if (pGroup->m_iESP & ESPEnum::IntelReturnTime && pIntel->m_nFlagStatus() == TF_FLAGINFO_DROPPED)
		{
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::vformat(XS("Return {:.1f}s"), std::make_format_args( pIntel->m_flResetTime() - TICKS_TO_TIME(I::ClientState->m_ClockDriftMgr.m_nServerTick))).c_str(), Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
		}
...
static inline void StoreMisc(CBaseEntity* pEntity, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, EntityCache_t>& mCache)
{
	...
	if (pGroup->m_iESP & ESPEnum::Name)
	{
		const char* sName = XS("Unknown");
		switch (pEntity->GetClassID())
		{
		case ETFClassID::CTFBaseBoss: sName = XS("Boss"); break;
		case ETFClassID::CTFTankBoss: sName = XS("Tank"); break;
		case ETFClassID::CMerasmus: sName = XS("Merasmus"); break;
		case ETFClassID::CEyeballBoss: sName = XS("Monoculus"); break;
		case ETFClassID::CHeadlessHatman: sName = XS("Horseless Headless Horsemann"); break;
		case ETFClassID::CZombie: sName = XS("Skeleton"); break;
		case ETFClassID::CBaseAnimating:
		{
			...
			else if (H::Entities.IsPowerup(uHash))
			{
				...
				switch (uHash)
				{
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_agility.mdl")): sName = XS("Agility"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_crit.mdl")): sName = XS("Revenge"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_defense.mdl")): sName = XS("Resistance"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_haste.mdl")): sName = XS("Haste"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_king.mdl")): sName = XS("King"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_knockout.mdl")): sName = XS("Knockout"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_plague.mdl")): sName = XS("Plague"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_precision.mdl")): sName = XS("Precision"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_reflect.mdl")): sName = XS("Reflect"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_regen.mdl")): sName = XS("Regeneration"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_strength.mdl")): sName = XS("Strength"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_supernova.mdl")): sName = XS("Supernova"); break;
				case FNV1A::Hash32Const(XS("models/pickups/pickup_powerup_vampire.mdl")): sName = XS("Vampire");
				}
			}
			break;
		}
		case ETFClassID::CTFAmmoPack: sName = XS("Ammo"); break;
		case ETFClassID::CCurrencyPack: sName = XS("Money"); break;
		case ETFClassID::CTFGenericBomb:
		case ETFClassID::CTFPumpkinBomb: sName = XS("Bomb"); break;
		case ETFClassID::CHalloweenGiftPickup: sName = XS("Gargoyle"); break;
		}
...
void CESP::DrawPlayers()
{
	...
	for (auto& [pEntity, tCache] : m_mPlayerCache)
	{
		...
		{
			const char* sTexture = XS("vgui/glyph_multiplayer.vtf");
			switch (tCache.m_iClassIcon)
			{
			case TF_CLASS_SCOUT: sTexture = XS("hud/leaderboard_class_scout.vtf"); break;
			case TF_CLASS_SOLDIER: sTexture = XS("hud/leaderboard_class_soldier.vtf"); break;
			case TF_CLASS_PYRO: sTexture = XS("hud/leaderboard_class_pyro.vtf"); break;
			case TF_CLASS_DEMOMAN: sTexture = XS("hud/leaderboard_class_demo.vtf"); break;
			case TF_CLASS_HEAVY: sTexture = XS("hud/leaderboard_class_heavy.vtf"); break;
			case TF_CLASS_ENGINEER: sTexture = XS("hud/leaderboard_class_engineer.vtf"); break;
			case TF_CLASS_MEDIC: sTexture = XS("hud/leaderboard_class_medic.vtf"); break;
			case TF_CLASS_SNIPER: sTexture = XS("hud/leaderboard_class_sniper.vtf"); break;
			case TF_CLASS_SPY: sTexture = XS("hud/leaderboard_class_spy.vtf"); break;
			}
