...
void CCameraWindow::RenderCustomView(void* ecx, const CViewSetup& pViewSetup, ITexture* pTexture)
{
	...
	static auto CViewRender_RenderView = U::Hooks.m_mHooks[XS("CViewRender_RenderView")];
...
void CCameraWindow::Initialize()
{
	if (!m_pCameraMaterial)
	{
		...
		kv->SetString(XS("$basetexture"), XS("m_pCameraTexture"));
		m_pCameraMaterial = F::Materials.Create(XS("CameraMaterial"), kv);
	}
