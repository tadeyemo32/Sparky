	...
	for (auto& [pEntity, pGroup] : F::Groups.GetGroup())
	{
			...
			&& (F::Backtrack.GetFakeLatency() || F::Backtrack.GetFakeInterp() > G::Lerp || F::Backtrack.GetWindow()))
		{
			...
			if (pWeapon && (pGroup->m_iBacktrack & BacktrackEnum::Always || G::PrimaryWeaponType != EWeaponType::PROJECTILE))
			{
				...
				if (G::PrimaryWeaponType == EWeaponType::MELEE && SDK::AttribHookValue(0, XS("speed_buff_ally"), pWeapon) > 0)
					bShowFriendly = true;
	...
	auto drawModel = [&](Vec3& vOrigin, const DrawModelState_t& pState, const ModelRenderInfo_t& pInfo, matrix3x4* pBoneToWorld, float flBlend)
		{
			...
			static auto IVModelRender_DrawModelExecute = U::Hooks.m_mHooks[XS("IVModelRender_DrawModelExecute")];
...
void CGlow::RenderViewmodel(void* ecx, int flags)
{
	...
	static auto CBaseAnimating_InternalDrawModel = U::Hooks.m_mHooks[XS("CBaseAnimating_InternalDrawModel")];
...
void CGlow::Initialize()
{
	...
	if (!m_pMatGlowColor)
	{
		m_pMatGlowColor = I::MaterialSystem->FindMaterial(XS("dev/glow_color"), TEXTURE_GROUP_OTHER);
	...
	{
		KeyValues* kv = new KeyValues(XS("UnlitGeneric"));
		kv->SetString(XS("$basetexture"), XS("RenderBuffer1"));
		kv->SetString(XS("$additive"), XS("1"));
		m_pMatHaloAddToScreen = F::Materials.Create(XS("MatHaloAddToScreen"), kv);
	}

	if (!m_pMatBlurX)
	{
		KeyValues* kv = new KeyValues(XS("BlurFilterX"));
		kv->SetString(XS("$basetexture"), XS("RenderBuffer1"));
		m_pMatBlurX = F::Materials.Create(XS("MatBlurX"), kv);
	}

	if (!m_pMatBlurY)
	{
		KeyValues* kv = new KeyValues(XS("BlurFilterY"));
		kv->SetString(XS("$basetexture"), XS("RenderBuffer2"));
		m_pMatBlurY = F::Materials.Create(XS("MatBlurY"), kv);
		m_pBloomAmount = m_pMatBlurY->FindVar(XS("$bloomamount"), nullptr);
	}
}
