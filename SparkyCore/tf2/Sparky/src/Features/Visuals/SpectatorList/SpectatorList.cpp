...
bool CSpectatorList::GetSpectators(CTFPlayer* pTarget)
{
	...
	for (int n = 1; n <= I::EngineClient->GetMaxClients(); n++)
	{
		...
		{
			m_vSpectators.emplace_back(F::PlayerUtils.GetPlayerName(n, pResource->GetName(n)), XS("possible"), -1.f, false, n);
			continue;
		}
		...
		const char* sMode = XS("possible");
		if (!pPlayer->IsDormant())
		{
			switch (iObserverMode)
			{
			case OBS_MODE_FIRSTPERSON: sMode = XS("1st"); break;
			case OBS_MODE_THIRDPERSON: sMode = XS("3rd"); break;
			default: continue;
			}
		}
...
void CSpectatorList::Draw(CTFPlayer* pLocal)
{
	...
	H::Draw.StringOutlined(fFont, x, y, Vars::Menu::Theme::Accent.Value, Vars::Menu::Theme::Background.Value, align, XSFMT(XS("Spectating {}:"), sName).c_str());
	for (auto& tSpectator : m_vSpectators)
	{
		...
		else if (FNV1A::Hash32(tSpectator.m_sMode) == FNV1A::Hash32Const(XS("1st")))
			tColor = tColor.Lerp({ 255, 150, 0, 255 }, 0.5f);

		if (tSpectator.m_flRespawnIn != -1.f)
			H::Draw.StringOutlined(fFont, x + iconOffset, y, tColor, Vars::Menu::Theme::Background.Value, align, XSFMT(XS("{} ({} - respawn {}s)"), tSpectator.m_sName, tSpectator.m_sMode, tSpectator.m_flRespawnIn).c_str());
		else
			H::Draw.StringOutlined(fFont, x + iconOffset, y, tColor, Vars::Menu::Theme::Background.Value, align, XSFMT(XS("{} ({})"), tSpectator.m_sName, tSpectator.m_sMode).c_str());
	}
}