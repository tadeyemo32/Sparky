...
bool CProjectileSimulation::GetInfoMain(CTFPlayer* pPlayer, CTFWeaponBase* pWeapon, Vec3 vAngles, ProjectileInfo& tProjInfo, int iFlags, float flAutoCharge)
{
	...
	static auto sv_gravity = H::ConVars.FindVar(XS("sv_gravity"));
	...
	if (Vars::Visuals::Trajectory::Override.Value)
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("custom")), vPos, vAngle, { Vars::Visuals::Trajectory::Hull.Value, Vars::Visuals::Trajectory::Hull.Value, Vars::Visuals::Trajectory::Hull.Value }, Vars::Visuals::Trajectory::Speed.Value, Vars::Visuals::Trajectory::Gravity.Value, Vars::Visuals::Trajectory::LifeTime.Value };
		return true;
	}
	...
	if (!bInterp && G::CurrentUserCmd)
	{
		...
		case TF_WEAPON_GRAPPLINGHOOK:
		{
			...
			SDK::RandomSeed(SDK::SeedFileLineHash(MD5_PseudoRandom(iCmdNum) & 0x7FFFFFFF, XS("SelectWeightedSequence"), 0));
			for (int i = 0; i < 6; ++i)
				SDK::RandomFloat();
	...
	case TF_WEAPON_ROCKETLAUNCHER_DIRECTHIT:
	{
		SDK::GetProjectileFireSetup(pPlayer, vAngles, { 23.5f, int(SDK::AttribHookValue(0, XS("centerfire_projectile"), pWeapon)) == 1 ? 0.f : 12.f, bDucking ? 8.f : -3.f }, vPos, vAngle, bRedirect ? 2000.f : 0.f, 0.1f, bInterp);
		
		float flSpeed = pPlayer->InCond(TF_COND_RUNE_PRECISION) ? 3000.f : SDK::AttribHookValue(1100.f, XS("mult_projectile_speed"), pWeapon);
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_rocket.mdl")), vPos, vAngle, { 0.f, 0.f, 0.f }, flSpeed, 0.f };
		return true;
	}
	case TF_WEAPON_PARTICLE_CANNON:
	case TF_WEAPON_RAYGUN:
	case TF_WEAPON_DRG_POMSON:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_drg_ball.mdl")), vPos, vAngle, bCowMangler ? Vec3() : Vec3(1.f, 1.f, 1.f), flSpeed, 0.f };
		return true;
	}
	...
	case TF_WEAPON_CANNON:
	{
		...
		float flMortar = bCannon ? SDK::AttribHookValue(0.f, XS("grenade_launcher_mortar_mode"), pWeapon) : 0.f;

		SDK::GetProjectileFireSetup(pPlayer, vAngles, { 16.f, 8.f, -6.f }, vPos, vAngle, 0.f, 0.f, bInterp);
		
		float flSpeed = SDK::AttribHookValue(pPlayer->InCond(TF_COND_RUNE_PRECISION) ? 3000.f : SDK::AttribHookValue(1200.f, XS("mult_projectile_speed"), pWeapon), XS("mult_projectile_range"), pWeapon);
		float flLifetime = flMortar
			...
			: SDK::AttribHookValue(2.f, XS("fuse_mult"), pWeapon);
		auto uType = bCannon ? FNV1A::Hash32Const(XS("models/weapons/w_models/w_cannonball.mdl")) : FNV1A::Hash32Const(XS("models/weapons/w_models/w_grenade_grenadelauncher.mdl"));
	...
	case TF_WEAPON_PIPEBOMBLAUNCHER:
	{
		...
		float flCharge = flAutoCharge > 0.f && SDK::AttribHookValue(1, XS("mult_dmg"), pWeapon)
			? SDK::AttribHookValue(4.f, XS("stickybomb_charge_rate"), pWeapon) * flAutoCharge
		...
		float flSpeed = SDK::AttribHookValue(bMaxSpeed ? 2400.f : Math::RemapVal(flCharge, 0.f, SDK::AttribHookValue(4.f, XS("stickybomb_charge_rate"), pWeapon), 900.f, 2400.f), XS("mult_projectile_range"), pWeapon);
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_stickybomb.mdl")), vPos, vAngle, { 6.f, 6.f, 6.f }, flSpeed, 1.f };
		return true;
	}
	case TF_WEAPON_FLAREGUN:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_flaregun_shell.mdl")), vPos, vAngle, { 0.f, 0.f, 0.f }, SDK::AttribHookValue(2000.f, XS("mult_projectile_speed"), pWeapon), 0.3f * flGravity };
		return true;
	}
	case TF_WEAPON_FLAREGUN_REVENGE:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_flaregun_shell.mdl")), vPos, vAngle, { 0.f, 0.f, 0.f }, 3000.f, 0.45f * flGravity };
		return true;
	}
	case TF_WEAPON_COMPOUND_BOW:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_arrow.mdl")), vPos, vAngle, { 1.f, 1.f, 1.f }, flSpeed, flGravity, 10.f /*arrows have some lifetime check for whatever reason*/ };
		return true;
	}
	case TF_WEAPON_CROSSBOW:
	case TF_WEAPON_SHOTGUN_BUILDING_RESCUE:
	{
		...
		auto uType = bCrossbow ? FNV1A::Hash32Const(XS("models/weapons/w_models/w_syringe_proj.mdl")) : FNV1A::Hash32Const(XS("models/weapons/w_models/w_repair_claw.mdl"));
	...
	case TF_WEAPON_SYRINGEGUN_MEDIC:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/w_models/w_syringe_proj.mdl")), vPos, vAngle, { 1.f, 1.f, 1.f }, 1000.f, 0.3f * flGravity };
		return true;
	}
	case TF_WEAPON_FLAMETHROWER:
	{
		static auto tf_flamethrower_boxsize = H::ConVars.FindVar(XS("tf_flamethrower_boxsize"));
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("particles/flamethrower.pcf")), vPos, vAngle, { flHull, flHull, flHull }, 1000.f, 0.f, 0.285f };
		return true;
	}
	case TF_WEAPON_FLAME_BALL:
	{
		static auto tf_fireball_speed = H::ConVars.FindVar(XS("tf_fireball_speed"));
		static auto tf_fireball_distance = H::ConVars.FindVar(XS("tf_fireball_distance"));
		static auto tf_fireball_max_lifetime = H::ConVars.FindVar(XS("tf_fireball_max_lifetime"));
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/c_models/c_flameball/c_flameball.mdl")), vPos, vAngle, { 1.f, 1.f, 1.f /*damaging hull much bigger, shouldn't matter here*/ }, flSpeed, 0.f, flLifeTime };
		return true;
	}
	case TF_WEAPON_CLEAVER:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/workshop_partner/weapons/c_models/c_sd_cleaver/c_sd_cleaver.mdl")), vPos, vAngle, { 1.f, 1.f, 10.f /*weird, probably still inaccurate*/ }, 3000.f, 1.f, 2.2f };
		return true;
	}
	case TF_WEAPON_BAT_WOOD:
	case TF_WEAPON_BAT_GIFTWRAP:
	{
		static auto tf_scout_stunball_base_speed = H::ConVars.FindVar(XS("tf_scout_stunball_base_speed"));
		...
		auto uHash = bWrapAssassin ? FNV1A::Hash32Const(XS("models/weapons/c_models/c_xms_festive_ornament.mdl")) : FNV1A::Hash32Const(XS("models/weapons/w_models/w_baseball.mdl"));
	...
	case TF_WEAPON_JAR_MILK:
	{
		...
		uint32_t uType = uType = FNV1A::Hash32Const(XS("models/weapons/c_models/urinejar.mdl"));
		switch (pWeapon->m_iItemDefinitionIndex())
		{
		case Scout_s_MadMilk: uType = FNV1A::Hash32Const(XS("models/workshop/weapons/c_models/c_madmilk/c_madmilk.mdl")); break;
		case Sniper_s_TheSelfAwareBeautyMark: uType = FNV1A::Hash32Const(XS("models/weapons/c_models/c_breadmonster/c_breadmonster.mdl")); break;
		case Scout_s_MutatedMilk: uType = FNV1A::Hash32Const(XS("models/weapons/c_models/c_breadmonster/c_breadmonster_milk.mdl")); break;
		}
	...
	case TF_WEAPON_JAR_GAS:
	{
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/c_models/c_gascan/c_gascan.mdl")), vPos, vAngle, { 3.f, 3.f, 3.f }, 2000.f, 1.f, 2.2f };
		return true;
	}
	case TF_WEAPON_GRAPPLINGHOOK:
	{
		static auto tf_grapplinghook_projectile_speed = H::ConVars.FindVar(XS("tf_grapplinghook_projectile_speed"));
		static auto tf_grapplinghook_max_distance = H::ConVars.FindVar(XS("tf_grapplinghook_max_distance"));
		...
		tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/c_models/c_grapple_proj/c_grapple_proj.mdl")), vPos, vAngle, { 1.2f, 1.2f, 1.2f }, flSpeed, 0.f, flLifetime };
		return true;
	}
	case TF_WEAPON_LUNCHBOX:
	{
		...
		case Heavy_s_SecondBanana:
		{
			...
			tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const(XS("models/weapons/c_models/c_sandwich/c_sandwich.mdl")), vPos, vAngle, { 17.f, 17.f, 7.f }, 500.f, 1.f * flGravity };
			return true;
		}
		}
		break;
...
bool CProjectileSimulation::Initialize(ProjectileInfo& tProjInfo, bool bSimulate, bool bWorld)
{
	...
	//set drag
	{
		...
		{
		case FNV1A::Hash32Const(XS("custom")):
			...
			break;
		case FNV1A::Hash32Const(XS("models/weapons/w_models/w_grenade_grenadelauncher.mdl")):
		case FNV1A::Hash32Const(XS("models/workshop/weapons/c_models/c_quadball/w_quadball_grenade.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.003902f, 0.009962f, 0.009962f };
			vAngDragBasis = { 0.003618f, 0.001514f, 0.001514f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/w_models/w_stickybomb.mdl")):
		case FNV1A::Hash32Const(XS("models/workshop/weapons/c_models/c_kingmaker_sticky/w_kingmaker_stickybomb.mdl")):
		case FNV1A::Hash32Const(XS("models/weapons/w_models/w_stickybomb_d.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.007491f, 0.007491f, 0.007306f };
			vAngDragBasis = { 0.002777f, 0.002842f, 0.002812f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/w_models/w_stickybomb2.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.007394f, 0.007394f, 0.007100f };
			vAngDragBasis = { 0.002654f, 0.002717f, 0.002708f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/w_models/w_cannonball.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.020971f, 0.019420f, 0.020971f };
			vAngDragBasis = { 0.012997f, 0.013496f, 0.013714f };
			break;
		case FNV1A::Hash32Const(XS("models/workshop_partner/weapons/c_models/c_sd_cleaver/c_sd_cleaver.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.022287f, 0.005208f, 0.110697f };
			vAngDragBasis = { 0.013982f, 0.043243f, 0.003465f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/w_models/w_baseball.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.009000f /*0.006645f*/, 0.006581f, 0.006710f};
			vAngDragBasis = { 0.002233f, 0.002246f, 0.002206f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/c_models/c_xms_festive_ornament.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.013500f /*0.010867f*/, 0.0108727f, 0.010804f };
			vAngDragBasis = { 0.002081f, 0.002162f, 0.002069f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/c_models/urinejar.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.005127f, 0.002925f, 0.004337f };
			vAngDragBasis = { 0.000641f, 0.001350f, 0.000717f };
			break;
		case FNV1A::Hash32Const(XS("models/workshop/weapons/c_models/c_madmilk/c_madmilk.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.005514f, 0.002313f, 0.005558f };
			vAngDragBasis = { 0.000684f, 0.001439f, 0.000680f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/c_models/c_breadmonster/c_breadmonster.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.002208f, 0.001640f, 0.002187f };
			vAngDragBasis = { 0.000799f, 0.001515f, 0.000879f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/c_models/c_breadmonster/c_breadmonster_milk.mdl")):
			flDrag = 1.f;
			vDragBasis = { 0.002335f, 0.001960f, 0.001920f };
			vAngDragBasis = { 0.000672f, 0.001064f, 0.000747f };
			break;
		case FNV1A::Hash32Const(XS("models/weapons/c_models/c_gascan/c_gascan.mdl")):
			flDrag = 1.f;
	...
	//set position and velocity
	{
		...
		if (!bWorld)
		{
			...
			if (tProjInfo.m_uType == FNV1A::Hash32Const(XS("models/weapons/c_models/c_sandwich/c_sandwich.mdl")))
			...
			case FNV1A::Hash32Const(XS("models/weapons/w_models/w_cannonball.mdl")):
				...
				if (!(tProjInfo.m_iFlags & ProjSimEnum::Interp) && G::CurrentUserCmd)
				{
					...
					if (!tProjInfo.m_pWeapon || !SDK::AttribHookValue(0, XS("grenade_no_spin"), tProjInfo.m_pWeapon))
			...
			case FNV1A::Hash32Const(XS("models/weapons/c_models/c_gascan/c_gascan.mdl")):
				...
				break;
			case FNV1A::Hash32Const(XS("particles/flamethrower.pcf")):
