...
void CMovementSimulation::Store(MoveStorage& tMoveStorage)
{
	...
	copy.TransferData(XS("MovementSimulationStore"), tMoveStorage.m_pPlayer->entindex(), pMap);
}
...
void CMovementSimulation::Reset(MoveStorage& tMoveStorage)
{
	if (tMoveStorage.m_pData)
	{
		...
		copy.TransferData(XS("MovementSimulationReset"), tMoveStorage.m_pPlayer->entindex(), pMap);
...
bool CMovementSimulation::Initialize(CBaseEntity* pEntity, MoveStorage& tMoveStorage, bool bHitchance, bool bStrafe)
{
	...
	if (bHitchance && bCalculated && !pPlayer->m_vecVelocity().IsZero() && Vars::Aimbot::Projectile::HitChance.Value)
	{
		...
		{
			SDK::Output(XS("MovementSimulation"), XSFMT(XS("Hitchance ({}% < {}%)"), flCurrentChance * 100, Vars::Aimbot::Projectile::HitChance.Value).c_str(), { 80, 200, 120 }, Vars::Debug::Logging.Value);
...
static inline float GetFrictionScale(float flVelocityXY, float flTurn, float flVelocityZ, float flMin = 50.f, float flMax = 150.f)
{
	...
	static auto sv_airaccelerate = H::ConVars.FindVar(XS("sv_airaccelerate"));
...
void CMovementSimulation::GetAverageYaw(MoveStorage& tMoveStorage, int iSamples)
{
	...
	size_t i = 1; for (; i < iSamples; i++)
	{
		...
		SDK::Output(XS("GetYawDifference"), XSFMT(XS("{} ({}): {}, {}"), i, iTicks, flYaw, bResult).c_str(), { 50, 127, 75 }, Vars::Debug::Logging.Value);
		if (!bResult)
			break;
	...
	*/
	...
	SDK::Output(XS("MovementSimulation"), XSFMT(XS("flAverageYaw calculated to {} from {} ({}) {}"), flAverageYaw, iTicks, iMinimum, pPlayer->entindex() == I::EngineClient->GetLocalPlayer() ? XS("(local)") : "").c_str(), { 100, 255, 150 }, Vars::Debug::Logging.Value);
}
