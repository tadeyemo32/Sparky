...
void COutput::Event(IGameEvent* pEvent, uint32_t uHash, CTFPlayer* pLocal)
{
	...
	case FNV1A::Hash32Const(XS("vote_cast")): // Voting
	{
		...
		OutputInfo(Vars::Logging::VoteCast::LogTo.Value, XS("Vote Cast"),
			XSFMT(XS("{}{} voted {}"), (bSameTeam ? "" : XS("[Enemy] ")), (sName), (bVotedYes ? XS("Yes") : XS("No"))).c_str(),
			XSFMT(XS("{}{}{}\x1 voted {}{}"), (bSameTeam ? "" : XS("[Enemy] ")), (s_sYellow), (sName), (bVotedYes ? s_sGreen : s_sRed), (bVotedYes ? XS("Yes") : XS("No"))).c_str()
		);
	...
	case FNV1A::Hash32Const(XS("player_changeclass")): // Class change
	{
		...
		OutputInfo(Vars::Logging::ClassChange::LogTo.Value, XS("Class Change"),
			XSFMT(XS("{}{} changed class to {}"), (bSameTeam ? "" : XS("[Enemy] ")), (sName), (SDK::GetClassByIndex(pEvent->GetInt(XS("class"))))).c_str(),
			XSFMT(XS("{}{}{}\x1 changed class to {}{}"), (bSameTeam ? "" : XS("[Enemy] ")), (s_sYellow), (sName), (s_sYellow), (SDK::GetClassByIndex(pEvent->GetInt(XS("class"))))).c_str()
		);
	...
	case FNV1A::Hash32Const(XS("player_hurt")): // Damage
	{
		...
		OutputInfo(Vars::Logging::Damage::LogTo.Value, XS("Damage"),
			XSFMT(XS("Hit {} for {} damage ({} / {}{})"), (sName), (nDamage), (nHealth), (iMaxHealth), (bCrit ? XS(", crit") : bMinicrit ? XS(", minicrit") : "")).c_str(),
			XSFMT(XS("Hit {}{}\x1 for {}{} damage{} ({} / {}{})"), (s_sYellow), (sName), (s_sRed), (nDamage), (s_sYellow), (nHealth), (iMaxHealth), (bCrit ? XS(", crit") : bMinicrit ? XS(", minicrit") : "")).c_str()
		);
...
void COutput::UserMessage(bf_read& msgData)
{
	...
	OutputInfo(Vars::Logging::VoteStart::LogTo.Value, XS("Vote Start"),
		XSFMT(XS("{}{} called a vote on {}"), (bSameTeam ? "" : XS("[Enemy] ")), (sCallerName), (sTargetName)).c_str(),
		XSFMT(XS("{}{}{}\x1 called a vote on {}{}"), (bSameTeam ? "" : XS("[Enemy] ")), (s_sYellow), (sCallerName), (s_sYellow), (sTargetName)).c_str()
	);
}
...
void COutput::CheatDetection(const char* sName, const char* sAction, const char* sReason)
{
	...
	OutputInfo(Vars::Logging::CheatDetection::LogTo.Value, XS("Cheat Detection"),
		XSFMT(XS("{} {} for {}"), (sName), (sAction), (sReason)).c_str(),
		XSFMT(XS("{}{}\x1 {} for {}{}"), (s_sYellow), (sName), (sAction), (s_sYellow), (sReason)).c_str()
	);
}
...
void COutput::TagsOnJoin(const char* sName, uint32_t uAccountID)
{
	...
	case 1:
	{
		...
		sChatText = XSFMT(XS("{}{}"), pColorTag.first, pColorTag.second);
		break;
	}
	case 2:
	{
		auto& pColorTag1 = vColorsTags.front(), &pColorTag2 = vColorsTags.back();
		sOutputText = XSFMT(XS("{} and {}"), pColorTag1.second, pColorTag2.second);
		sChatText = XSFMT(XS("{}{}\x1 and {}{}"), pColorTag1.first, pColorTag1.second, pColorTag2.first, pColorTag2.second);
		break;
	}
	default:
	{
		for (auto it = vColorsTags.begin(); it != vColorsTags.end(); it++)
		{
			...
			{
				sOutputText += XSFMT(XS("{}, "), pColorTag.second);
				sChatText += XSFMT(XS("{}{}\x1, "), pColorTag.first, pColorTag.second);
			}
			else
			{
				sOutputText += XSFMT(XS("and {}"), pColorTag.second);
				sChatText += XSFMT(XS("and {}{}"), pColorTag.first, pColorTag.second);
			}
		}
	}
	}
	...
	OutputInfo(Vars::Logging::Tags::LogTo.Value, XS("Tags"),
		XSFMT(XS("{} has the {} {}"), (sName), (vColorsTags.size() == 1 ? XS("tag") : XS("tags")), (sOutputText)).c_str(),
		XSFMT(XS("{}{}\x1 has the {} {}"), (s_sYellow), (sName), (vColorsTags.size() == 1 ? XS("tag") : XS("tags")), (sChatText)).c_str()
	);
}
void COutput::TagsChanged(const char* sName, const char* sAction, const char* sColor, const char* sTag)
{
	...
	OutputInfo(Vars::Logging::Tags::LogTo.Value, XS("Tags"),
		XSFMT(XS("{} tag {} {} {}"), (sAction), (sTag), (uHash == FNV1A::Hash32Const(XS("Added")) ? XS("to") : XS("from")), (sName)).c_str(),
		XSFMT(XS("{} tag {}{}\x1 {} {}{}"), (sAction), (sColor), (sTag), (uHash == FNV1A::Hash32Const(XS("Added")) ? XS("to") : XS("from")), (s_sYellow), (sName)).c_str()
	);
}
...
void COutput::AliasOnJoin(const char* sName, uint32_t uAccountID)
{
	...
	OutputInfo(Vars::Logging::Tags::LogTo.Value, XS("Aliases"),
		XSFMT(XS("{} has the alias \"{}\""), (sName), (sAlias)).c_str(),
		XSFMT(XS("{}{}\x1 has the alias \"{}{}\x1\""), (s_sYellow), (sName), (s_sYellow), (sAlias)).c_str()
	);
}
void COutput::AliasChanged(const char* sName, const char* sAction, const char* sAlias)
{
	...
	auto uHash = FNV1A::Hash32(sAction);
	OutputInfo(Vars::Logging::Tags::LogTo.Value, XS("Aliases"),
		XSFMT(XS("{} {}'s alias {} \"{}\""), (sAction), (sName), (uHash == FNV1A::Hash32Const(XS("Changed")) ? XS("to") : XS("of")), (sAlias)).c_str(),
		XSFMT(XS("{} {}{}\x1's alias {} \"{}{}\x1\""), (sAction), (s_sYellow), (sName), (uHash == FNV1A::Hash32Const(XS("Changed")) ? XS("to") : XS("of")), (s_sYellow), (sAlias)).c_str()
	);
}

void COutput::ReportResolver(int iIndex, const char* sAction, const char* sAxis, float flValue)
{
	ReportResolver(iIndex, sAction, sAxis, XSFMT(XS("{}"), flValue).c_str());
}
void COutput::ReportResolver(int iIndex, const char* sAction, const char* sAxis, bool bValue)
{
	ReportResolver(iIndex, sAction, sAxis, XSFMT(XS("{}"), bValue).c_str());
}
void COutput::ReportResolver(int iIndex, const char* sAction, const char* sAxis, const char* sValue)
{
	...
	OutputInfo(Vars::Logging::Tags::LogTo.Value, XS("Resolver"),
		XSFMT(XS("{} {} of {} to {}"), (sAction), (sAxis), (sName), (sValue)).c_str(),
		XSFMT(XS("{} {}{}\x1 of {}{}\x1 to {}{}\x1"), (sAction), (s_sYellow), (sAxis), (s_sYellow), (sName), (s_sYellow), (sValue)).c_str()
	);
}
