...
namespace ImGui
{
	...
	inline void PopDisabled(int count = 1)
	{
		...
		if (iSize < count)
		{
			IM_ASSERT_USER_ERROR(0, XS("Calling PopDisabled() too many times: stack underflow."));
			count = iSize;
		}
	...
	inline void PopTransparent(int count = 1, int iPopAlpha = false)
	{
		...
		if (iSize < count)
		{
			IM_ASSERT_USER_ERROR(0, XS("Calling PopTransparent() too many times: stack underflow."));
			count = iSize;
		}
		for (int i = 0; i < count; i++)
	...
	inline std::string TruncateText(const std::string& sText, int iPixels, ImFont* pFont = nullptr, const char* sEnd = "...", size_t* pLength = nullptr)
	{
		...
		size_t i = 0;
		while (FCalcTextSize(XSFMT(XS("{}{}{}"), sTruncated, sText[i + 1], sEnd).c_str(), pFont).x < iPixels)
		{
			sTruncated += sText[i++];
			if (i >= sText.length())
			{
	...
	inline std::deque<std::string> WrapText(const std::string& sText, int iPixels, ImFont* pFont = nullptr)
	{
		...
		begin:
			...
			if (sWord[0] == ' ' || sWord[0] == '\n' || sWord[0] == '\t')
				sWord.erase(0, 1);
			if (sSeparator != XS("\n") && FCalcTextSize(XSFMT(XS("{}{}{}"), vWrapped.back(), sSeparator, sWord).c_str(), pFont).x < iPixels)
			{
				vWrapped.back() += XSFMT(XS("{}{}"), sSeparator, sWord);
				iWord++;
			}
			else
			{
				...
				{
					size_t i = 0;
					vWrapped.back() += TruncateText(sWord, iPixels, pFont, XS("-"), &i);
	...
	inline bool FInputText(const char* sLabel, std::string& sText, float flWidth = H::Draw.Scale(150), int iFlags = ImGuiInputTextFlags_None, ImGuiInputTextCallback fCallback = nullptr)
	{
		...
		bool bReturn = InputText(XSFMT(XS("##{}"), sLabel).c_str(), &sText, iFlags | ImGuiInputTextFlags_NoKeyboardNavigate, fCallback);
	...
	inline bool IconButton(const char* sIcon, float flSize = H::Draw.Scale(24), ImVec4 tColor = { 1, 1, 1, -1 }, bool* pHovered = nullptr)
	{
		...
		ImVec2 vOriginalPos = GetCursorPos();
		bool bReturn = Button(XSFMT(XS("{}##{}:{}"), sIcon, vOriginalPos.x, vOriginalPos.y).c_str(), { flSize, flSize });
	...
	{	// WOW fuck this
		...
		{
			IM_ASSERT_USER_ERROR(0, XS("FTabs() vIcons size mismatch to vEntries."));
			return false;
		}
		...
		for (size_t i = bReverse ? vEntries.size() - 1 : 0; i < vEntries.size(); (bReverse ? i-- : i++))
		{
			for (size_t j = bReverse ? vEntries[i].size() - 1 : 0; j < vEntries[i].size(); (bReverse ? j-- : j++))
			{
				...
				if (Button(XSFMT(XS("##{}"), sEntry).c_str(), vNewSize) && !iTabState && pVar)
				{
					*pVar = int(!j ? i : j - 1);
					bChanged = true;
	...
	inline bool FToggle(const char* sLabel, bool* pVar, int iFlags = FToggleEnum::None, bool* pHovered = nullptr)
	{
		...
		bool bReturn = Button(XSFMT(XS("##{}"), sLabel).c_str(), vSize);
		if (pHovered)
			*pHovered = IsItemHovered();
	...
	inline bool FSlider(const char* sLabel, float* pVar1, float* pVar2, float flMin, float flMax, float flStep = 1.f, const char* fmt = "%g", int iFlags = FSliderEnum::None, bool* pHovered = nullptr)
	{
...
#endif
		{
			auto uHash2 = FNV1A::Hash32Const(XSFMT(XS("{}## Text"), sLabel).c_str());
			...
			else
			{
				...
				InputText(XS("##SliderText"), &sInput, ImGuiInputTextFlags_CharsDecimal); sText = sInput;
			...
			if (!Disabled)
			{
				...
				else if (IsItemClicked())
				{
					...
					sInput = XSFMT(XS("{}"), *pVar); // would use to_string but i don't like its formatting
		...
		Button(XS("##"), { vMaxs.x - vMins.x + 12, 14 }); // don't drag it around
		SetCursorPos(vOriginalPos);
		AddRowSize(vOriginalPos, vSize);
		DebugDummy({ vSize.x, GetRowSize(vSize.y) });
	...
	inline bool FDropdown(const char* sLabel, int* pVar, std::vector<const char*> vEntries, std::vector<int> vValues = {}, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, const char* sDefaultPreview = "None", bool* pHovered = nullptr, int* pModified = nullptr)
	{
		...
		if (vValues.empty())
		{
			...
			{
				if (FNV1A::Hash32(sEntry) == FNV1A::Hash32Const(XS("##Divider")))
					continue;
		...
		if (!(iFlags & FDropdownEnum::Multi) || *pVar)
		{
			...
			{
				while (FNV1A::Hash32(vEntries[i]) == FNV1A::Hash32Const(XS("##Divider")))
					i++;

				if (iFlags & FDropdownEnum::Multi && *pVar & iValue)
					sPreview += XSFMT(XS("{}, "), StripDoubleHash(vEntries[i]).c_str());
		...
		if (iFlags & FDropdownEnum::Right)
			SameLine(vSize.x + GetStyle().WindowPadding.x * 2.f);
		iSizeOffset = strstr(sLabel, XS("## Bind")) ? 0 : H::Draw.Scale(iSizeOffset, Scale_Round);
		vSize.x += iSizeOffset;
		...
		bool bActive = BeginCombo(XSFMT(XS("##{}"), sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge);
		if (bActive)
		{
			...
			int i = 0; for (auto& sEntry : vEntries)
			{
				...
				if (iFlags & FDropdownEnum::Multi)
				{
					...
					ImVec2 vOriginalPos2 = GetCursorPos();
					if (FSelectable(XSFMT(XS("##{}{}"), sEntry, i).c_str(), nullptr, 0, bFlagActive, ImGuiSelectableFlags_DontClosePopups))
					{
						if (bFlagActive)
							*pVar &= ~vValues[i];
						else
				...
				else
				{
					...
					if (FSelectable(XSFMT(XS("##{}{}"), sEntry, i).c_str(), nullptr, 0, *pVar == vValues[i]))
	...
	inline bool FSDropdown(const char* sLabel, std::string* pVar, std::vector<const char*> vEntries = {}, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		if (BeginCombo(XSFMT(XS("##{}"), sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge))
		{
			if (!mActiveMap[uHash])
				sPreview = sInput = "";
			...
			if (!IsAnyItemActive())
				SetKeyboardFocusHere();
			bool bEnter = InputText(XS("##FSDropdown"), &sInput, ImGuiInputTextFlags_EnterReturnsTrue);
			if (sInput != sTab)
			{
				sPreview = sInput;
				sTab = XS("\n");
			...
			if (!vValid.empty() && FNV1A::Hash32(vValid.front().c_str()) == FNV1A::Hash32Const(XS("##Divider")))
				vValid.pop_front();
			if (!vValid.empty() && FNV1A::Hash32(vValid.back().c_str()) == FNV1A::Hash32Const(XS("##Divider")))
				vValid.pop_back();
			if (!vValid.empty())
			{
				if (U::KeyHandler.Pressed(VK_TAB))
				{
					...
					while (true)
					{
						...
						if (iIndex < 0)
							...
							iIndex -= int(vValid.size());
						if (FNV1A::Hash32(vValid[iIndex].c_str()) == FNV1A::Hash32Const(XS("##Divider")))
							continue;
				...
				for (int i = 0; i < vValid.size(); i++)
				{
					...
					if (FNV1A::Hash32(sEntry.c_str()) == FNV1A::Hash32Const(XS("##Divider")))
					{
						if (!bDivider)
						{
							ImVec2 vDrawPos = GetDrawPos(); float flPosY = GetCursorPosY();
					...
					ImVec2 vOriginalPos3 = GetCursorPos();
					if (FSelectable(XSFMT(XS("##{}{}"), sEntry, i).c_str(), nullptr, 0, bActive))
						*pVar = sEntry, bReturn = true;
	...
	inline bool ColorPicker(const char* sLabel, Color_t* pColor, bool bTooltip = true, ImVec2 vSize = { H::Draw.Scale(12), H::Draw.Scale(12) })
	{
		...
		bool bReturn = ColorEdit4(XSFMT(XS("##{}"), sLabel).c_str(), &tTempColor.x, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoBorder | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_LargeAlphaGrid | ImGuiColorEditFlags_NoRoundRestrict, vSize);
		if (bReturn)
	...
	inline std::string VK2STR(short key)
	{
		switch (key)
		{
		case 0x0: return XS("none");
		case VK_LBUTTON: return XS("mouse1");
		case VK_RBUTTON: return XS("mouse2");
		case VK_MBUTTON: return XS("mouse3");
		case VK_XBUTTON1: return XS("mouse4");
		case VK_XBUTTON2: return XS("mouse5");
		case VK_SHIFT: return XS("shift");
		case VK_LSHIFT: return XS("lshift");
		case VK_RSHIFT: return XS("rshift");
		case VK_CONTROL: return XS("control");
		case VK_LCONTROL: return XS("lcontrol");
		case VK_RCONTROL: return XS("rcontrol");
		case VK_MENU: return XS("alt");
		case VK_LMENU: return XS("lalt");
		case VK_RMENU: return XS("ralt");
		case VK_NUMPAD0: return XS("num0");
		case VK_NUMPAD1: return XS("num1");
		case VK_NUMPAD2: return XS("num2");
		case VK_NUMPAD3: return XS("num3");
		case VK_NUMPAD4: return XS("num4");
		case VK_NUMPAD5: return XS("num5");
		case VK_NUMPAD6: return XS("num6");
		case VK_NUMPAD7: return XS("num7");
		case VK_NUMPAD8: return XS("num8");
		case VK_NUMPAD9: return XS("num9");
		case VK_ADD: return XS("num+");
		case VK_SUBTRACT: return XS("num-");
		case VK_MULTIPLY: return XS("num*");
		case VK_DIVIDE: return XS("num/");
		case VK_DECIMAL: return XS("num.");
		case VK_INSERT: return XS("insert");
		case VK_DELETE: return XS("delete");
		case VK_PRIOR: return XS("pgup");
		case VK_NEXT: return XS("pgdown");
		case VK_HOME: return XS("home");
		case VK_END: return XS("end");
		case VK_CLEAR: return XS("clear");
		case VK_UP: return XS("up");
		case VK_DOWN: return XS("down");
		case VK_LEFT: return XS("left");
		case VK_RIGHT: return XS("right");
		case VK_ESCAPE: return XS("escape");
		case VK_F13: return XS("f13");
		case VK_F14: return XS("f14");
		case VK_F15: return XS("f15");
		case VK_F16: return XS("f16");
		case VK_F17: return XS("f17");
		case VK_F18: return XS("f18");
		case VK_F19: return XS("f19");
		case VK_F20: return XS("f20");
		case VK_F21: return XS("f21");
		case VK_F22: return XS("f22");
		case VK_F23: return XS("f23");
		case VK_F24: return XS("f24");
		case VK_LWIN:
		case VK_RWIN: return XS("windows");
		case VK_APPS: return XS("contextmenu");
		case VK_PAUSE: return XS("pause");
		case VK_VOLUME_MUTE: return XS("mute");
		case VK_VOLUME_DOWN: return XS("volumedown");
		case VK_VOLUME_UP: return XS("volumeup");
		case VK_MEDIA_STOP: return XS("stop");
		case VK_MEDIA_PLAY_PAUSE: return XS("pause");
		case VK_MEDIA_PREV_TRACK: return XS("previous");
		case VK_MEDIA_NEXT_TRACK: return XS("next");
		}

		std::string str = XS("unknown");
	...
	inline void FKeybind(const char* sLabel, int& iOutput, int iFlags = FKeybindEnum::None, std::vector<int> vIgnore = { Vars::Menu::PrimaryKey[DEFAULT_BIND], Vars::Menu::SecondaryKey[DEFAULT_BIND] }, ImVec2 vSize = { 0, 30 }, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		if (GetActiveID() == uId)
		{
			...
			bool bHovered = false;
			FButton(XSFMT(XS("{}: ..."), sLabel).c_str(), iFlags | FButtonEnum::NoUpper, vSize, iSizeOffset, nullptr, &bHovered);
			if (pHovered)
				*pHovered = bHovered;
		...
		else if (FButton(XSFMT(XS("{}: {}"), sLabel, VK2STR(iOutput)).c_str(), iFlags | FButtonEnum::NoUpper, vSize, iSizeOffset, nullptr, pHovered))
			SetActiveID(uId, GetCurrentWindow());
	...
	inline bool FMDropdown(const char* sLabel, std::vector<std::pair<std::string, Color_t>>* pVar, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		{
			if (FNV1A::Hash32(mat.m_sName.c_str()) != FNV1A::Hash32Const(XS("None")))
		...
		std::vector<std::string> vEntries = { XS("Original") };
		...
		{
			for (auto& pair : *pVar)
				sPreview += XSFMT(XS("{}, "), pair.first.c_str());
			sPreview.pop_back(); sPreview.pop_back();
		}
		...
		if (BeginCombo(XSFMT(XS("##{}"), sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge))
		{
			...
			for (int i = 0; i < vEntries.size(); i++)
			{
				...
				if (bFlagActive) // do here so as to not sink input
				{
					...
					ColorPicker(XSFMT(XS("MaterialColor{}"), iEntry).c_str(), &it->second->second, false);
					SetCursorPos(vOriginalPos2);
				}
				...
				if (bFlagActive)
				{
					...
					TextColored(F::Render.Accent, XS("%i"), iEntry);
				}
				SetCursorPos(vOriginalPos3);
			}
			PopStyleVar();
	...
	{
		TextUnformatted(XSFMT(XS("Bind '{}'"), sBind).c_str());
		...
		for (auto _iBind : vValues)
			vStrings.push_back(XSFMT(XS("{}## Bind{}"), _iBind != DEFAULT_BIND && _iBind < F::Binds.m_vBinds.size() ? F::Binds.m_vBinds[_iBind].m_sName : sBind, _iBind));
		for (auto& sEntry : vStrings)
			vEntries.push_back(sEntry.c_str());

		vEntries.push_back(XS("new bind"));
		vValues.push_back(-1);

		int iModified = -2;
		if (FDropdown(XS("Bind"), &iBind, vEntries, vValues, FDropdownEnum::Modifiable, -85, XS("None"), nullptr, &iModified))
		{
		...
		FDropdown(XS("Type"), &tBind.m_iType, { XS("Key"), XS("Class"), XS("Weapon type"), XS("Item slot"), XS("Misc") }, {}, FDropdownEnum::Left, 0, XS("None"), &bHovered);
		bLastHovered |= bHovered;
		switch (tBind.m_iType)
		{
		case BindEnum::Key: FDropdown(XS("Behavior"), &tBind.m_iInfo, { XS("Hold"), XS("Toggle"), XS("Double click") }, {}, FDropdownEnum::Right, 0, XS("None"), &bHovered); break;
		case BindEnum::Class: FDropdown(XS("Class"), &tBind.m_iInfo, { XS("Scout"), XS("Soldier"), XS("Pyro"), XS("Demoman"), XS("Heavy"), XS("Engineer"), XS("Medic"), XS("Sniper"), XS("Spy") }, {}, FDropdownEnum::Right, 0, XS("None"), &bHovered); break;
		case BindEnum::WeaponType: FDropdown(XS("Weapon type"), &tBind.m_iInfo, { XS("Hitscan"), XS("Projectile"), XS("Melee"), XS("Throwable") }, {}, FDropdownEnum::Right, 0, XS("None"), &bHovered); break;
		case BindEnum::ItemSlot: FDropdown(XS("Item slot"), &tBind.m_iInfo, { XS("1"), XS("2"), XS("3"), XS("4"), XS("5"), XS("6"), XS("7"), XS("8"), XS("9") }, {}, FDropdownEnum::Right, 0, XS("None"), &bHovered); break;
		...
		{
			FKeybind(XS("Key"), tBind.m_iKey, FKeybindEnum::None, { Vars::Menu::PrimaryKey[DEFAULT_BIND], Vars::Menu::SecondaryKey[DEFAULT_BIND] }, { 0, 30 }, 0, &bHovered);
			bLastHovered |= bHovered;
		}
	...
	{ \
		...
		bool bHovered = false; \
		bool bReturn = function(XSFMT(XS("{}## {}"), sLabel, var.Name()).c_str(), arguments, &bHovered); \
		FSet(var, val); \
		if (pHovered) \
			*pHovered = bHovered; \
		...
		{	/*probably a better way to do this*/ \
			...
			{ \
				...
				if (FNV1A::Hash32Const(#function) == FNV1A::Hash32Const(XS("FColorPicker"))) \
				...
				val = staticVal; \
				function(XSFMT(XS("{}## Bind"), var.m_vNames.front()).c_str(), arguments, &bHovered); \
				bLastHovered |= bHovered; \
				staticVal = val; \
				PopTransparent(2); \
				EndPopup(); \
	...
	WRAPPER(FDropdown, int, VA_LIST(int iFlags = 0, int iSizeOffset = 0), VA_LIST(&val, var.m_vValues, {}, iFlags, iSizeOffset, var.m_sExtra ? var.m_sExtra : XS("None")))
	WRAPPER(FDropdown, int, VA_LIST(std::vector<const char*> vEntries, std::vector<int> vValues = {}, int iFlags = 0, int iSizeOffset = 0), VA_LIST(&val, vEntries, vValues, iFlags, iSizeOffset, var.m_sExtra ? var.m_sExtra : XS("None")))
