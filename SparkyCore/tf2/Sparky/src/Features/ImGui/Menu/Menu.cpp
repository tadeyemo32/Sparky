...
void CMenu::DrawMenu()
{
	...
	if (Begin(XS("Main"), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground))
	{
		...
		if (CurrentBind != DEFAULT_BIND) // bind
		{
			...
			FText(XS("Editing bind"), 0, F::Render.FontRegular);
		...
		FTabs(
			{
				{ XS("AIMBOT"), XS("GENERAL"), XS("DRAW") },
				{ XS("VISUALS"), XS("ESP"), XS("MISC##"), XS("MENU") },
				{ XS("MISC"), XS("MAIN"), XS("HVH") },
				{ XS("LOGS"), XS("PLAYERLIST"), XS("SETTINGS##"), XS("OUTPUT") },
				{ XS("SETTINGS"), XS("CONFIG"), XS("BINDS"), XS("MATERIALS"), XS("EXTRA") }
			},
		...
		FInputText(XS("Search..."), sSearch, H::Draw.Scale(123), ImGuiInputTextFlags_None);
		...
		if (BeginChild(XS("Page"), { vWindowSize.x - H::Draw.Scale(flSideSize), vWindowSize.y }, ImGuiChildFlags_AlwaysUseWindowPadding))
		{
			if (!bSearch)
			{
				switch (iTab)
...
void CMenu::MenuAimbot(int iTab)
{
	...
	case 0:
	{
		if (BeginTable(XS("AimbotTable"), 2))
		{
			/* Column 1 */
			TableNextColumn();
			{
				if (Section(XS("General")))
				{
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section(XS("##Debug Backtrack")))
					{
						FSlider(Vars::Backtrack::Offset);
					} EndSection();
				}
				if (Section(XS("Healing")))
				{
			...
			TableNextColumn();
			{
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section(XS("##Debug Hitscan"), -8))
					{
						FDropdown(Vars::Aimbot::Hitscan::PeekCheck, FDropdownEnum::None, 0, &Hovered); FTooltip(XS("This should stay as doubletap only or off if you want to be able to target hitboxes other than the highest priority one"), Hovered);
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section(XS("##Debug Projectile")))
					{
						FText(XS("Ground"));
						Divider();
						...
						FSlider(Vars::Aimbot::Projectile::SplashRotateX, FSliderEnum::Left, Vars::Aimbot::Projectile::SplashRotateX[DEFAULT_BIND] < 0.f ? XS("random") : XS("%g"));
						FSlider(Vars::Aimbot::Projectile::SplashRotateY, FSliderEnum::Right, Vars::Aimbot::Projectile::SplashRotateY[DEFAULT_BIND] < 0.f ? XS("random") : XS("%g"));
						...
						FDropdown(Vars::Aimbot::Projectile::RocketSplashMode, FDropdownEnum::Right, 0, &Hovered); FTooltip(XS("Special splash type for rockets, more expensive"), Hovered);
	...
	case 1:
	{
		if (BeginTable(XS("DrawTable"), 2))
		{
			/* Column 1 */
			TableNextColumn();
			{
				if (Section(XS("Line"), 8))
				{
...
void CMenu::MenuVisuals(int iTab)
{
	...
	case 0:
	{
		...
		if (Section(XS("Groups")))
		{
			static std::string sStaticName;

			PushDisabled(F::Groups.m_vGroups.size() >= sizeof(int) * 8); // for active groups flags
			{
				FSDropdown(XS("Name"), &sStaticName, {}, FDropdownEnum::Left | FSDropdownEnum::AutoUpdate, -H::Draw.Unscale(FCalcTextSize(XS("CREATE")).x) - 36);

				PushDisabled(Disabled || sStaticName.empty());
				{
					if (FButton(XS("Create"), FButtonEnum::Fit | FButtonEnum::SameLine, { 0, 40 }))
					{
			...
			for (auto it = F::Groups.m_vGroups.begin(); it < F::Groups.m_vGroups.end();)
			{
				...
				bool bClicked = Button(std::vformat(XS("##{}"), std::make_format_args( iGroup)).c_str(), { flWidth, flHeight });
				bool bPopup = IsItemClicked(ImGuiMouseButton_Right);

				if (bClicked)
					iCurrentGroup = iGroup;
				else if (bPopup)
					OpenPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( iGroup)).c_str());
				else if (iDragging == -1 && IsItemHovered() && IsMouseDown(ImGuiMouseButton_Left))
					iDragging = iGroup;

				if (FBeginPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( iGroup)).c_str()))
				{
...

					{
						...
						bool bEnter = FInputText(XS("Name..."), sInput, H::Draw.Scale(284), ImGuiInputTextFlags_EnterReturnsTrue);
						if (!IsItemFocused())
					...
					if (FButton(XS("Duplicate")))
					{
						auto it2 = F::Groups.m_vGroups.insert(it + 1, tGroup);
						it2->m_sName += XS(" duplicate");
					}
		...
		if (!F::Groups.m_vGroups.empty()
			&& BeginTable(XS("VisualsESPTable"), 2))
		{
			...
			TableNextColumn();
			{
				if (Section(XS("Color"), 8))
				{
					FColorPicker(XS("Group color"), &tGroup.m_tColor, FColorPickerEnum::Left);
					FToggle(XS("Tags override color"), &tGroup.m_bTagsOverrideColor, FToggleEnum::Right);
				} EndSection();
				if (Section(XS("Targets")))
				{
					FDropdown(XS("Targets"), &tGroup.m_iTargets, { XS("Players"), XS("Buildings"), XS("Projectiles"), XS("Ragdolls"), XS("Objective"), XS("NPCs"), XS("Health"), XS("Ammo"), XS("Money"), XS("Powerups"), XS("Spellbook"), XS("Bombs"), XS("Gargoyle"), XS("##Divider"), XS("Fake angle"), XS("Viewmodel weapon"), XS("Viewmodel hands") }, {}, FDropdownEnum::Multi);
				} EndSection();
				if (Section(XS("Conditions")))
				{
					FDropdown(XS("Conditions"), &tGroup.m_iConditions, { XS("Enemy"), XS("Team"), XS("BLU"), XS("RED"), XS("##Divider"), XS("Local"), XS("Friends"), XS("Party"), XS("Priority"), XS("Target"), XS("##Divider"), XS("Dormant") }, {}, FDropdownEnum::Multi);
					Divider(H::Draw.Scale(), H::Draw.Scale(8), -H::Draw.Scale());
					PushTransparent(!(tGroup.m_iTargets & TargetsEnum::Players));
					{
						FDropdown(XS("Players"), &tGroup.m_iPlayers, { XS("Scout"), XS("Soldier"), XS("Pyro"), XS("Demoman"), XS("Heavy"), XS("Engineer"), XS("Medic"), XS("Sniper"), XS("Spy"), XS("##Divider"), XS("Invulnerable"), XS("Crits"), XS("Invisible"), XS("Disguise"), XS("Hurt") }, {}, FDropdownEnum::Multi, 0, XS("All"));
					}
					PopTransparent();
					PushTransparent(!(tGroup.m_iTargets & TargetsEnum::Buildings));
					{
						FDropdown(XS("Buildings"), &tGroup.m_iBuildings, { XS("Sentry"), XS("Dispenser"), XS("Teleporter"), XS("##Divider"), XS("Hurt") }, {}, FDropdownEnum::Multi, 0, XS("All"));
					}
					PopTransparent();
					PushTransparent(!(tGroup.m_iTargets & TargetsEnum::Projectiles));
					{
						FDropdown(XS("Projectiles"), &tGroup.m_iProjectiles, { XS("Rocket"), XS("Sticky"), XS("Pipe"), XS("Arrow"), XS("Heal"), XS("Flare"), XS("Fire"), XS("Repair"), XS("Cleaver"), XS("Milk"), XS("Jarate"), XS("Gas"), XS("Bauble"), XS("Baseball"), XS("Energy"), XS("Short circuit"), XS("Meteor shower"), XS("Lightning"), XS("Fireball"), XS("Bomb"), XS("Bats"), XS("Pumpkin"), XS("Monoculus"), XS("Skeleton"), XS("Misc"), XS("##Divider"), XS("Crit"), XS("Minicrit") }, {}, FDropdownEnum::Multi, 0, XS("All"));
					}
					PopTransparent();
				} EndSection();
			}
			...
			TableNextColumn();
			{
				if (Section(XS("ESP")))
				{
					std::vector<const char*> vEntries = { XS("Name"), XS("Box"), XS("Distance") };
					std::vector<int> vValues = { ESPEnum::Name, ESPEnum::Box, ESPEnum::Distance };
					if (tGroup.m_iTargets & TargetsEnum::Players)
					{
						vEntries.insert(vEntries.end(), { XS("Bones") });
						vValues.insert(vValues.end(), { ESPEnum::Bones });
					}
					if (tGroup.m_iTargets & (TargetsEnum::Players | TargetsEnum::Buildings))
					{
						vEntries.insert(vEntries.end(), { XS("Health bar"), XS("Health text") });
						vValues.insert(vValues.end(), { ESPEnum::HealthBar, ESPEnum::HealthText });
					}
					if (tGroup.m_iTargets & TargetsEnum::Players)
					{
						vEntries.insert(vEntries.end(), { XS("Uber bar"), XS("Uber text"), XS("Class icon"), XS("Class text"), XS("Weapon icon"), XS("Weapon text"), XS("Priority"), XS("Labels"), XS("Buffs"), XS("Debuffs") });
					...
					{
						vEntries.insert(vEntries.end(), { XS("Flags") });
						vValues.insert(vValues.end(), { ESPEnum::Flags });
					}
					if (tGroup.m_iTargets & TargetsEnum::Players)
					{
						vEntries.insert(vEntries.end(), { XS("Lag compensation"), XS("Ping"), XS("KDR") });
				...
				if (Section(XS("Chams")))
				{
					...
					{
						FMDropdown(XS("Visible material"), &tGroup.m_tChams.Visible, FDropdownEnum::Left);
						FMDropdown(XS("Occluded material"), &tGroup.m_tChams.Occluded, FDropdownEnum::Right);
					}
					else
						FMDropdown(XS("Material"), &tGroup.m_tChams.Visible);
				} EndSection();
				if (Section(XS("Glow"), 8))
				{
					PushTransparent(!tGroup.m_tGlow.Stencil);
					{
						FSlider(XS("Stencil scale"), &tGroup.m_tGlow.Stencil, 0, 10, 1, XS("%i"), FSliderEnum::Left | FSliderEnum::Min);
					}
					PopTransparent();
					PushTransparent(!tGroup.m_tGlow.Blur);
					{
						FSlider(XS("Blur scale"), &tGroup.m_tGlow.Blur, 0.f, 10.f, 1.f, XS("%g"), FSliderEnum::Right | FSliderEnum::Min | FSliderEnum::Precision);
					}
					PopTransparent();
				} EndSection();
				if (Section(XS("Misc"), 8))
				{
					FToggle(XS("Offscreen arrows"), &tGroup.m_bOffscreenArrows, FToggleEnum::Left);
					SameLine(GetWindowWidth() - H::Draw.Scale(33));
					if (IconButton(ICON_MD_KEYBOARD_ARROW_DOWN))
						OpenPopup(XS("OffscreenArrows"));

					FToggle(XS("Pickup timer"), &tGroup.m_bPickupTimer);

					FToggle(XS("Backtrack"), &tGroup.m_iBacktrack, BacktrackEnum::Enabled, FToggleEnum::Left);
					SameLine(GetWindowWidth() - H::Draw.Scale(33));
					if (IconButton(ICON_MD_KEYBOARD_ARROW_DOWN))
						OpenPopup(XS("Backtrack"));

					FToggle(XS("Trajectory"), &tGroup.m_iTrajectory, TrajectoryEnum::Enabled, FToggleEnum::Left);
					SameLine(GetWindowWidth() - H::Draw.Scale(33));
					if (IconButton(ICON_MD_KEYBOARD_ARROW_DOWN))
						OpenPopup(XS("Trajectory"));

					FToggle(XS("Sightlines"), &tGroup.m_iSightlines, SightlinesEnum::Enabled, FToggleEnum::Left);
					SameLine(GetWindowWidth() - H::Draw.Scale(33));
					if (IconButton(ICON_MD_KEYBOARD_ARROW_DOWN))
						OpenPopup(XS("Sightlines"));

					SetNextWindowSize({ H::Draw.Scale(300), 0 });
					if (FBeginPopup(XS("OffscreenArrows")))
					{
						FSlider(XS("Offset"), &tGroup.m_iOffscreenArrowsOffset, 0, 1000, 25, XS("%i"), FSliderEnum::Precision);
						FSlider(XS("Max distance"), &tGroup.m_flOffscreenArrowsMaxDistance, 0.f, 5000.f, 50.f, XS("%g"), FSliderEnum::Min | FSliderEnum::Precision);

						EndPopup();
					}

					SetNextWindowSize({ H::Draw.Scale(300), 0 });
					if (FBeginPopup(XS("Backtrack")))
					{
						SetCursorPosY(GetCursorPosY() - H::Draw.Scale(8));

						FDropdown(XS("##Draw"), &tGroup.m_iBacktrack, { XS("Last"), XS("First"), XS("##Divider"), XS("Always") }, { BacktrackEnum::Last, BacktrackEnum::First, BacktrackEnum::Always }, FDropdownEnum::Multi | FDropdownEnum::NoSanitization, 0, XS("All"));

						FMDropdown(XS("Material"), &tGroup.m_vBacktrackChams, FDropdownEnum::Left);
						SetCursorPos({ GetWindowWidth() / 2 + GetStyle().WindowPadding.x / 2, GetCursorPosY() - H::Draw.Scale(32) });
						FToggle(XS("Ignore Z"), &tGroup.m_iBacktrack, BacktrackEnum::IgnoreZ, FToggleEnum::Left);

						SetCursorPosY(GetCursorPosY() + H::Draw.Scale(8));
						PushTransparent(!tGroup.m_tBacktrackGlow.Stencil);
						{
							FSlider(XS("Stencil scale## Backtrack"), &tGroup.m_tBacktrackGlow.Stencil, 0, 10, 1, XS("%i"), FSliderEnum::Left | FSliderEnum::Min);
						}
						PopTransparent();
						PushTransparent(!tGroup.m_tBacktrackGlow.Blur);
						{
							FSlider(XS("Blur scale## Backtrack"), &tGroup.m_tBacktrackGlow.Blur, 0.f, 10.f, 1.f, XS("%g"), FSliderEnum::Right | FSliderEnum::Min | FSliderEnum::Precision);
						}
						PopTransparent();
					...
					SetNextWindowSize({ H::Draw.Scale(300), 0 });
					if (FBeginPopup(XS("Trajectory")))
					{
						SetCursorPosY(GetCursorPosY() - H::Draw.Scale(8));

						FDropdown(XS("Flags"), &tGroup.m_iTrajectory, { XS("Predict"), XS("##Divider"), XS("Radius"), XS("Trace"), XS("Sphere"), XS("##Divider"), XS("Path") }, { TrajectoryEnum::Predict, TrajectoryEnum::Radius, TrajectoryEnum::Trace, TrajectoryEnum::Sphere, TrajectoryEnum::Path }, FDropdownEnum::Multi | FDropdownEnum::NoSanitization);
						FToggle(XS("Ignore Z"), &tGroup.m_iTrajectory, SightlinesEnum::IgnoreZ);

						EndPopup();
					}

					SetNextWindowSize({ H::Draw.Scale(300), 0 });
					if (FBeginPopup(XS("Sightlines")))
					{
						FToggle(XS("Ignore Z"), &tGroup.m_iSightlines, SightlinesEnum::IgnoreZ);
	...
	case 1:
	{
		if (BeginTable(XS("VisualsMiscTable"), 2))
		{
			/* Column 1 */
			TableNextColumn();
			{
	...
	case 2:
	{
		if (BeginTable(XS("MenuTable"), 2))
		{
			/* Column 1 */
			TableNextColumn();
			{
				if (Section(XS("Settings"), 8))
				{
...
void CMenu::MenuMisc(int iTab)
{
	...
	case 0:
	{
		if (BeginTable(XS("MiscTable"), 2))
		{
			/* Column 1 */
			TableNextColumn();
			{
				if (Section(XS("Movement")))
				{
				...
				if (Section(XS("Mann vs. Machine"), 8))
				{
			...
			TableNextColumn();
			{
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section(XS("##Debug AntiCheat")))
					{
						FToggle(Vars::Misc::Game::AntiCheatCritHack);
					} EndSection();
				}
				if (Section(XS("Queueing")))
				{
	...
	case 1:
	{
		if (BeginTable(XS("HvHTable"), 2))
		{
			/* Column 1 */
			TableNextColumn();
			{
				if (Section(XS("Doubletap"), 8))
				{
			...
			TableNextColumn();
			{
				...
				if (Section(XS("Auto Peek"), 8))
				{
					FToggle(Vars::AutoPeek::Enabled);
				} EndSection();
				if (Section(XS("Cheater Detection")))
				{
...
void CMenu::MenuLogs(int iTab)
{
	...
	case 0:
	{
				...
				auto drawPlayer = [&](const ListPlayer& tPlayer, int x, int y)
					{
						...
						if (!tPlayer.m_bFake)
						{
							...
							if (BeginChild(std::vformat(XS("TagBar{}"), std::make_format_args( tPlayer.m_iUserID)).c_str(), { flWidth - lOffset - H::Draw.Scale(4), flHeight }, ImGuiWindowFlags_None, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBackground))
							{
								...
								if (int iParty = tPlayer.m_iParty)
								{
									...
									if (!--iParty)
										...
										vLabels.emplace_back(std::vformat(XS("{}: {}"), std::make_format_args( pTag->m_sName, iParty)), pTag->m_tColor.HueShift(iParty * 360.f / iPartyCount));
								}
								if (tPlayer.m_bF2P)
								{
						...
						SetCursorPos(vOriginalPos);
						Button(std::vformat(XS("##{}"), std::make_format_args( tPlayer.m_iUserID)).c_str(), { flWidth, flHeight });
						bPopup |= IsItemHovered() && IsMouseReleased(ImGuiMouseButton_Right);

						// popups
						if (bPopup)
							OpenPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( tPlayer.m_iUserID)).c_str());

						if (FBeginPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( tPlayer.m_iUserID)).c_str()))
						{
							...
							if (!tPlayer.m_bFake)
							{
								...
								if (FSelectable(XS("History")))
									I::SteamFriends->ActivateGameOverlayToWebPage(std::vformat(XS("https://steamhistory.net/id/{}"), std::make_format_args( CSteamID(tPlayer.m_uAccountID, k_EUniversePublic, k_EAccountTypeIndividual).ConvertToUint64())).c_str());
							}
							...
							if (!I::EngineClient->IsPlayingDemo() && FBeginMenu(XS("Votekick")))
							{
								if (FSelectable(XS("No reason")))
									I::ClientState->SendStringCmd(std::vformat(XS("callvote Kick \"{} other\""), std::make_format_args( tPlayer.m_iUserID)).c_str());
								if (FSelectable(XS("Cheating")))
									I::ClientState->SendStringCmd(std::vformat(XS("callvote Kick \"{} cheating\""), std::make_format_args( tPlayer.m_iUserID)).c_str());
								if (FSelectable(XS("Idle")))
									I::ClientState->SendStringCmd(std::vformat(XS("callvote Kick \"{} idle\""), std::make_format_args( tPlayer.m_iUserID)).c_str());
								if (FSelectable(XS("Scamming")))
									I::ClientState->SendStringCmd(std::vformat(XS("callvote Kick \"{} scamming\""), std::make_format_args( tPlayer.m_iUserID)).c_str());
							...
							if (!tPlayer.m_bFake)
							{
								...
								if (FBeginMenu(XS("Alias")))
								{
									bool bHasAlias = F::PlayerUtils.m_mPlayerAliases.contains(tPlayer.m_uAccountID);
									static std::string sInput = "";

									bool bEnter = FInputText(XS("Alias..."), sInput, H::Draw.Scale(150), ImGuiInputTextFlags_EnterReturnsTrue);
									if (!IsItemFocused())
									...
									if (bEnter)
									{
										...
										{
											F::Output.AliasChanged(tPlayer.m_sName.c_str(), XS("Removed"), F::PlayerUtils.m_mPlayerAliases[tPlayer.m_uAccountID].c_str());
										...
										else if (!sInput.empty())
										{
											...
											F::Output.AliasChanged(tPlayer.m_sName.c_str(), bHasAlias ? XS("Changed") : XS("Added"), sInput.c_str());
										}
									}
							...
							if (Vars::Resolver::Enabled.Value && !tPlayer.m_bLocal && !I::EngineClient->IsPlayingDemo())
							{
								if (FBeginMenu(XS("Set yaw")))
								{
									...
									for (auto& [sYaw, flValue] : vYaws)
									{
										if (FSelectable(sYaw))
										{
											...
											{
											case FNV1A::Hash32Const(XS("Auto")):
									...
									for (auto& [sPitch, flValue] : vPitches)
									{
										if (FSelectable(sPitch))
										{
											...
											case FNV1A::Hash32Const(XS("Auto")):
												...
												break;
											case FNV1A::Hash32Const(XS("Inverse")):
								...
								if (FBeginMenu(XS("Set view")))
								{
									static std::vector<std::pair<const char*, bool>> vPitches = {
										{ XS("Offset from static view"), true },
										{ XS("Offset from view to local"), false }
									};
								...
								if (FBeginMenu(XS("Set minwalk")))
								{
									static std::vector<std::pair<const char*, bool>> vPitches = {
										{ XS("Minwalk on"), true },
										{ XS("Minwalk off"), false }
									};
									for (auto& [sPitch, bValue] : vPitches)
									{
										if (FSelectable(sPitch))
							...
							if (mParties.contains(tPlayer.m_iParty))
							{
								...
								TextColored(F::Render.Inactive.Value, XS("Partied:"));
		...
		if (Section(XS("Tags")))
		{
			...
			{
				FSDropdown(XS("Name"), &tTag.m_sName, {}, FDropdownEnum::Left | FSDropdownEnum::AutoUpdate, -10);
				FColorPicker(XS("Color"), &tTag.m_tColor, FColorPickerEnum::SameLine, {}, { H::Draw.Scale(10), H::Draw.Scale(40) });

				PushDisabled(iID == DEFAULT_TAG || iID == IGNORED_TAG);
				{
					int iLabel = Disabled ? 0 : tTag.m_bLabel;
					FDropdown(XS("Type"), &iLabel, { XS("Priority"), XS("Label") }, {}, FDropdownEnum::Right);
			...
			if (BeginWidgetTable(1, vTable))
			{
				PushTransparent(tTag.m_bLabel); // transparent if we want a label, user can still use to sort
				{
					...
					FSlider(XS("Priority"), &tTag.m_iPriority, -10, 10);
				}
				PopTransparent();
			} EndChild();
			...
			auto drawTag = [](std::vector<PriorityLabel_t>::iterator it, PriorityLabel_t& _tTag, int y)
				{
					...
					if (!_tTag.m_bLabel)
					{
						...
						FText(std::vformat(XS("{}"), std::make_format_args( _tTag.m_iPriority)).c_str());
					}
					...
					SetCursorPos(vOriginalPos);
					bool bClicked = Button(std::vformat(XS("##{}"), std::make_format_args( _tTag.m_sName)).c_str(), { flWidth, flHeight });
					bool bPopup = IsItemClicked(ImGuiMouseButton_Right);
					...
					if (bClicked)
					{
						...
						OpenPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( _iID)).c_str());
					else if (bDelete)
						OpenPopup(std::vformat(XS("DeleteTag{}"), std::make_format_args( _iID)).c_str());

					if (FBeginPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( _iID)).c_str()))
					{
						PushStyleVar(ImGuiStyleVar_ItemSpacing, { H::Draw.Scale(8), 0 });
						...
						PushDisabled(_iID == DEFAULT_TAG || _iID == IGNORED_TAG);
						{
							...
							if (FDropdown(XS("Type##"), &iLabel, { XS("Priority"), XS("Label") }))
								bSave = true;
						...
						PopDisabled();
						if (FSlider(XS("Priority##"), &_tTag2.m_iPriority, -10, 10))
							bSave = true;
					...
					else if (FBeginPopupModal(std::vformat(XS("DeleteTag{}"), std::make_format_args( _iID)).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
					{
						FText(std::vformat(XS("Do you really want to delete '{}'?"), std::make_format_args( _tTag.m_sName)).c_str());
						...
						if (FButton(XS("No"), FButtonEnum::Right | FButtonEnum::SameLine))
							CloseCurrentPopup();
			...
			PushStyleColor(ImGuiCol_Text, F::Render.Inactive.Value);
			SetCursorPos({ H::Draw.Scale(13), H::Draw.Scale(80) }); FText(XS("Priorities"));
			SetCursorPos({ GetWindowWidth() * 2 / 3 + H::Draw.Scale(9), H::Draw.Scale(80) }); FText(XS("Labels"));
			PopStyleColor();
		...
		} EndSection();
		{
			PushDisabled(F::PlayerUtils.m_bLoad);
			{
				...
				if (FButton(XS("Export"), FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					// this should be up2date anyways
					std::ifstream fStream(F::Configs.m_sCorePath + XS("Players.json"), std::ios_base::app);
					if (fStream.is_open())
					{
						std::string sString;
						{
						...
						SDK::Output(XS("Sparky"), XS("Copied playerlist to clipboard"), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}

				{
					...
					if (FButton(XS("Import"), FButtonEnum::Fit | FButtonEnum::SameLine))
					{
						try
						{
							...
							vTags = {
								...
								{ XS("Party"), { 100, 100, 255, 255 }, 0, true, false, true },
							...
							OpenPopup(XS("ImportPlayerlist"));
						}
						catch (...)
						{
							SDK::Output(XS("Sparky"), XS("Failed to import playerlist"), ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
						}
					}

					SetNextWindowSize({ H::Draw.Scale(300), 0 });
					if (FBeginPopupModal(XS("ImportPlayerlist"), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
					{
						FText(XS("Import"));
						FText(XS("As"), FTextEnum::Right | FTextEnum::SameLine);
						...
						for (int i = 0; i < vTags.size(); i++)
						{
							...
							std::vector<const char*> vEntries = { XS("None") };
							...
							{
								int iTo = iIDTo + 1;
								FDropdown(std::vformat(XS("##{}"), std::make_format_args( i)).c_str(), &iTo, vEntries, vValues, FSliderEnum::Right);
								iIDTo = iTo - 1;
							}
							PopTransparent();
						}

						if (FButton(XS("Import"), FButtonEnum::Left))
						{
							...
							F::PlayerUtils.m_bSave = true;
							SDK::Output(XS("Sparky"), XS("Imported playerlist"), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);

							CloseCurrentPopup();
						}
						if (FButton(XS("Cancel"), FButtonEnum::Right | FButtonEnum::SameLine))
							CloseCurrentPopup();
				...
				if (FButton(XS("Backup"), FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					try
					{
						int iBackupCount = 0;
						for (auto& tEntry : std::filesystem::directory_iterator(F::Configs.m_sCorePath))
						{
							...
							if (sConfigName.find(XS("Backup")) != std::string::npos)
								iBackupCount++;
						}
						std::filesystem::copy(
							F::Configs.m_sCorePath + XS("Players.json"),
							F::Configs.m_sCorePath + std::vformat(XS("Backup{}.json"), std::make_format_args( iBackupCount + 1)),
					...
					{
						SDK::Output(XS("Sparky"), XS("Failed to backup playerlist"), ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}
			}
			PopDisabled();

			if (FButton(XS("Folder"), FButtonEnum::Fit | FButtonEnum::SameLine))
	...
	case 2:
	{
		if (Section(XS("##Output"), false, GetWindowHeight() - GetStyle().WindowPadding.y * 2))
		{
			for (auto& tOutput : m_vOutput)
			{
				...
				if (IsItemHovered() && IsMouseReleased(ImGuiMouseButton_Right))
					OpenPopup(std::vformat(XS("Output{}"), std::make_format_args( tOutput.m_iID)).c_str());
				if (FBeginPopup(std::vformat(XS("Output{}"), std::make_format_args( tOutput.m_iID)).c_str()))
				{
					PushStyleVar(ImGuiStyleVar_ItemSpacing, { H::Draw.Scale(8), H::Draw.Scale(8) });

					if (FSelectable(XS("Copy")))
						SDK::SetClipboard(std::vformat(XS("{}{}{}"), std::make_format_args( tOutput.m_sFunction, tOutput.m_sFunction != "" ? XS(" ") : "", tOutput.m_sLog)));
...
void CMenu::MenuSettings(int iTab)
{
	...
	case 0:
	{
		if (BeginTable(XS("ConfigSettingsTable"), 2))
		{
			...
			*/
			...
			auto drawConfigs = [](std::string& sStaticName, bool bVisual = false)
				{
					...
					auto sType = !bVisual ? XS("Config") : XS("Visual");
					bool bNoSave = GetAsyncKeyState(VK_SHIFT) & 0x8000;

					FSDropdown(XS("Name"), &sStaticName, {}, FSDropdownEnum::AutoUpdate, -H::Draw.Unscale(FCalcTextSize(XS("CREATE")).x + FCalcTextSize(XS("FOLDER")).x) - 72);
					...
					PopDisabled();
					if (FButton(XS("Folder"), FButtonEnum::Fit | FButtonEnum::SameLine, { 0, 40 }))
					...
					for (auto& tEntry : std::filesystem::directory_iterator(sPath))
					{
						...
						if (FNV1A::Hash32(sName.c_str()) == FNV1A::Hash32Const(XS("default")))
							bDefaultFound = true;
					...
					{
						if (!bDefaultFound)
							F::Configs.SaveConfig(XS("default"));
						std::sort(vConfigs.begin(), vConfigs.end(), [&](const auto& a, const auto& b) -> bool
							{
								// override for default config
								if (FNV1A::Hash32(a.second.c_str()) == FNV1A::Hash32Const(XS("default")))
									return true;
								if (FNV1A::Hash32(b.second.c_str()) == FNV1A::Hash32Const(XS("default")))
									return false;
					...
					for (auto& [entry, sConfigName] : vConfigs)
					{
						...
						else if (bSave)
						{
							if (!bCurrentConfig || !bVisual && !F::Configs.m_sCurrentVisuals.empty())
								OpenPopup(std::vformat(XS("Save{}{}"), std::make_format_args( sType, sConfigName)).c_str());
							else if (!bVisual)
							...
							OpenPopup(std::vformat(XS("Remove{}{}"), std::make_format_args( sType, sConfigName)).c_str());

						if (FBeginPopupModal(std::vformat(XS("Save{}{}"), std::make_format_args( sType, sConfigName)).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
						{
							FText(std::vformat(XS("Do you really want to override '{}'?"), std::make_format_args( sConfigName)).c_str());
						...
						else if (FBeginPopupModal(std::vformat(XS("Remove{}{}"), std::make_format_args( sType, sConfigName)).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
						{
							FText(std::vformat(XS("Do you really want to remove '{}'?"), std::make_format_args( sConfigName)).c_str());

							PushDisabled(!bVisual && FNV1A::Hash32(sConfigName.c_str()) == FNV1A::Hash32Const(XS("default")));
							{
								if (FButton(XS("Yes, delete"), FButtonEnum::Fit))
								{
									if (!bVisual)
							...
							PopDisabled();
							if (FButton(XS("Yes, reset"), FButtonEnum::Fit | FButtonEnum::SameLine))
							{
								if (!bVisual)
							...
							if (FButton(XS("No"), FButtonEnum::Fit | FButtonEnum::SameLine))
								CloseCurrentPopup();
			...
			FText(XS("Built @ ") __DATE__ XS(", ") __TIME__ XS(", ") __CONFIGURATION__);
			//SetCursorPosX(GetCursorPosX() + GetStyle().WindowPadding.x);
			//FText(std::vformat("Time @ {}, {}", std::make_format_args( SDK::GetDate(), SDK::GetTime())).c_str());
			PopStyleColor();

			/* Column 2 */
			TableNextColumn();
			if (Section(XS("Visuals")))
			{
				static std::string sStaticName;
		...
		if (Section(XS("Binds")))
		{
			...
			if (BeginWidgetTable(0, vTable))
			{
				FSDropdown(XS("Name"), &tBind.m_sName, {}, FDropdownEnum::Left | FSDropdownEnum::AutoUpdate);
				{
					...
					if (FButton(std::vformat(XS("Parent: {}"), std::make_format_args( sParent)).c_str(), FButtonEnum::Right | FButtonEnum::SameLine | FButtonEnum::NoUpper, { 0, 40 }))
						bParent = 2;
				}
				...
				{
				case BindEnum::Key: FDropdown(XS("Behavior"), &tBind.m_iInfo, { XS("Hold"), XS("Toggle"), XS("Double click") }, {}, FDropdownEnum::Right); break;
				case BindEnum::Class: FDropdown(XS("Class"), &tBind.m_iInfo, { XS("Scout"), XS("Soldier"), XS("Pyro"), XS("Demoman"), XS("Heavy"), XS("Engineer"), XS("Medic"), XS("Sniper"), XS("Spy") }, {}, FDropdownEnum::Right); break;
				case BindEnum::WeaponType: FDropdown(XS("Weapon type"), &tBind.m_iInfo, { XS("Hitscan"), XS("Projectile"), XS("Melee"), XS("Throwable") }, {}, FDropdownEnum::Right); break;
				case BindEnum::ItemSlot: FDropdown(XS("Item slot"), &tBind.m_iInfo, { XS("1"), XS("2"), XS("3"), XS("4"), XS("5"), XS("6"), XS("7"), XS("8"), XS("9") }, {}, FDropdownEnum::Right); break;
				case BindEnum::Misc: FDropdown(XS("Misc"), &tBind.m_iInfo, { XS("Spectated"), XS("Spectated 1st"), XS("Spectated 3rd"), XS("##Divider"), XS("Zoomed"), XS("Aiming") }, {}, FDropdownEnum::Right); break;
				}
			} EndChild();
			...
			if (BeginWidgetTable(1, vTable))
			{
				...
				FDropdown(XS("While"), &iNot, { XS("Active"), XS("Not active") }, {}, FDropdownEnum::Left);
				tBind.m_bNot = iNot;
				FDropdown(XS("Visibility"), &tBind.m_iVisibility, { XS("Always"), XS("While active"), XS("Hidden") }, {}, FDropdownEnum::Right);
				if (tBind.m_iType == 0)
					FKeybind(XS("Key"), tBind.m_iKey, FButtonEnum::None, { Vars::Menu::PrimaryKey[DEFAULT_BIND], Vars::Menu::SecondaryKey[DEFAULT_BIND] }, { 0, 40 }, -96);
			...
			std::function<void(int, int)> getBinds = [&](int iParent, int x)
				{
					for (int _iBind = 0; _iBind < F::Binds.m_vBinds.size(); _iBind++)
					{
						...
						case BindEnum::Key:
							...
							{
							case BindEnum::KeyEnum::Hold: { sType = XS("hold"); break; }
							case BindEnum::KeyEnum::Toggle: { sType = XS("toggle"); break; }
							case BindEnum::KeyEnum::DoubleClick: { sType = XS("double"); break; }
							}
						...
						case BindEnum::Class:
							...
							switch (_tBind.m_iInfo)
							{
							case BindEnum::ClassEnum::Scout: { sInfo = XS("scout"); break; }
							case BindEnum::ClassEnum::Soldier: { sInfo = XS("soldier"); break; }
							case BindEnum::ClassEnum::Pyro: { sInfo = XS("pyro"); break; }
							case BindEnum::ClassEnum::Demoman: { sInfo = XS("demoman"); break; }
							case BindEnum::ClassEnum::Heavy: { sInfo = XS("heavy"); break; }
							case BindEnum::ClassEnum::Engineer: { sInfo = XS("engineer"); break; }
							case BindEnum::ClassEnum::Medic: { sInfo = XS("medic"); break; }
							case BindEnum::ClassEnum::Sniper: { sInfo = XS("sniper"); break; }
							case BindEnum::ClassEnum::Spy: { sInfo = XS("spy"); break; }
							}
							break;
						case BindEnum::WeaponType:
							...
							switch (_tBind.m_iInfo)
							{
							case BindEnum::WeaponTypeEnum::Hitscan: { sInfo = XS("hitscan"); break; }
							case BindEnum::WeaponTypeEnum::Projectile: { sInfo = XS("projectile"); break; }
							case BindEnum::WeaponTypeEnum::Melee: { sInfo = XS("melee"); break; }
							case BindEnum::WeaponTypeEnum::Throwable: { sInfo = XS("throwable"); break; }
							}
							break;
						case BindEnum::ItemSlot:
							sType = XS("slot");
							sInfo = std::vformat(XS("{}"), std::make_format_args( _tBind.m_iInfo + 1));
							break;
						case BindEnum::Misc:
							switch (_tBind.m_iInfo)
							{
							...
							case BindEnum::MiscEnum::SpectatedThird:
								...
								switch (_tBind.m_iInfo)
								{
								case BindEnum::MiscEnum::Spectated: { sInfo = XS("any"); break; }
								case BindEnum::MiscEnum::SpectatedFirst: { sInfo = XS("1st"); break; }
								case BindEnum::MiscEnum::SpectatedThird: { sInfo = XS("3rd"); break; }
								}
								break;
							...
							case BindEnum::MiscEnum::Aiming:
								...
								switch (_tBind.m_iInfo)
								{
								case BindEnum::MiscEnum::Zoomed: { sInfo = XS("zoomed"); break; }
								case BindEnum::MiscEnum::Aiming: { sInfo = XS("aiming"); break; }
								}
								break;
							}
							break;
						...
						if (_tBind.m_bNot && (_tBind.m_iType != BindEnum::Key || _tBind.m_iInfo == BindEnum::KeyEnum::Hold))
							sType = std::vformat(XS("not {}"), std::make_format_args( sType));
						...
						{	// don't indent too much
							auto sText = std::vformat(XS("-> {}"), std::make_format_args( x));
						...
						SetCursorPos(vOriginalPos);
						bool bClicked = Button(std::vformat(XS("##{}"), std::make_format_args( _iBind)).c_str(), { flWidth, flHeight });
						bool bPopup = IsItemClicked(ImGuiMouseButton_Right);
						...
						if (bClicked)
						{
							...
							OpenPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( _iBind)).c_str());
						else if (iDragging == -1 && IsItemHovered() && IsMouseDown(ImGuiMouseButton_Left))
						...
						else if (bDelete)
						{
							if (U::KeyHandler.Down(VK_SHIFT)) // allow user to quickly remove binds
								...
								OpenPopup(std::vformat(XS("DeleteBind{}"), std::make_format_args( _iBind)).c_str());
						}

						if (FBeginPopup(std::vformat(XS("RightClicked{}"), std::make_format_args( _iBind)).c_str()))
						{
							PushStyleVar(ImGuiStyleVar_ItemSpacing, { H::Draw.Scale(8), 0 });
							...
							FDropdown(XS("Type"), &_tBind.m_iType, { XS("Key"), XS("Class"), XS("Weapon type"), XS("Item slot"), XS("Misc") }, {}, FDropdownEnum::Left);
							switch (_tBind.m_iType)
							{
							case BindEnum::Key: FDropdown(XS("Behavior"), &_tBind.m_iInfo, { XS("Hold"), XS("Toggle"), XS("Double click") }, {}, FDropdownEnum::Right); break;
							case BindEnum::Class: FDropdown(XS("Class"), &_tBind.m_iInfo, { XS("Scout"), XS("Soldier"), XS("Pyro"), XS("Demoman"), XS("Heavy"), XS("Engineer"), XS("Medic"), XS("Sniper"), XS("Spy") }, {}, FDropdownEnum::Right); break;
							case BindEnum::WeaponType: FDropdown(XS("Weapon type"), &_tBind.m_iInfo, { XS("Hitscan"), XS("Projectile"), XS("Melee"), XS("Throwable") }, {}, FDropdownEnum::Right); break;
							case BindEnum::ItemSlot: FDropdown(XS("Item slot"), &_tBind.m_iInfo, { XS("1"), XS("2"), XS("3"), XS("4"), XS("5"), XS("6"), XS("7"), XS("8"), XS("9") }, {}, FDropdownEnum::Right); break;
							case BindEnum::Misc: FDropdown(XS("Misc"), &_tBind.m_iInfo, { XS("Spectated"), XS("Spectated 1st"), XS("Spectated 3rd"), XS("##Divider"), XS("Zoomed"), XS("Aiming") }, {}, FDropdownEnum::Right); break;
							}
							if (_tBind.m_iType == BindEnum::Key)
								FKeybind(XS("Key"), _tBind.m_iKey);

							PopStyleVar();
							EndPopup();
						}
						else if (FBeginPopupModal(std::vformat(XS("DeleteBind{}"), std::make_format_args( _iBind)).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
						{
							FText(std::vformat(XS("Do you really want to delete '{}'{}?"), std::make_format_args( _tBind.m_sName, F::Binds.HasChildren(_iBind) ? XS(" and all of its children") : "")).c_str());
	...
	case 2:
	{
		...
		if (!CurrentMaterial.empty())
			bTable = BeginTable(XS("MaterialsTable"), 2);
		{
			if (bTable)
			{
				TableSetupColumn(XS("MaterialsTable1"), ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(288));
				TableSetupColumn(XS("MaterialsTable2"), ImGuiTableColumnFlags_WidthFixed, GetWindowWidth());

				/* Column 1 */
				TableNextColumn();
			}

			if (Section(XS("Manager")))
			{
				static std::string sStaticName;
				...
				std::sort(vMaterials.begin(), vMaterials.end(), [&](const auto& a, const auto& b) -> bool
					{
						...
						if (FNV1A::Hash32(a.m_sName.c_str()) == FNV1A::Hash32Const(XS("None")))
							return true;
						if (FNV1A::Hash32(b.m_sName.c_str()) == FNV1A::Hash32Const(XS("None")))
							return false;
				...
				for (auto& tMaterial : vMaterials)
				{
					...
					if (!tMaterial.m_bLocked)
					{
						...
						if (IconButton(ICON_MD_DELETE))
							OpenPopup(std::vformat(XS("DeleteMat{}"), std::make_format_args( tMaterial.m_sName)).c_str());
						if (FBeginPopupModal(std::vformat(XS("DeleteMat{}"), std::make_format_args( tMaterial.m_sName)).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
						{
							FText(std::vformat(XS("Do you really want to delete '{}'?"), std::make_format_args( tMaterial.m_sName)).c_str());
			...
			if (bTable)
			{
				...
				if (CurrentMaterial.length())
				{
					...
					if (Section(XS("Editor"), 0, GetWindowHeight() - GetStyle().WindowPadding.y * 2, true))
					{
						// Toolbar
						if (!LockedMaterial)
						{
							if (FButton(XS("Save"), FButtonEnum::Fit))
							{
						...
						if (FButton(XS("Close"), FButtonEnum::Fit))
							CurrentMaterial = "";
						SetCursorPosY(H::Draw.Scale(52));
						PushStyleColor(ImGuiCol_Text, F::Render.Inactive.Value);
						FText(std::vformat(XS("{}: {}"), std::make_format_args( LockedMaterial ? XS("Viewing") : XS("Editing"), CurrentMaterial)).c_str(), FTextEnum::Right);
						PopStyleColor();
						...
						TextEditor.Render(XS("TextEditor"));
	...
	case 3:
	{
		...
		if (Section(XS("Buttons")))
		{
			if (FButton(XS("cl_fullupdate"), FButtonEnum::Left))
				I::EngineClient->ClientCmd_Unrestricted(XS("cl_fullupdate"));
			if (FButton(XS("retry"), FButtonEnum::Right | FButtonEnum::SameLine))
				I::EngineClient->ClientCmd_Unrestricted(XS("retry"));
			if (FButton(XS("Console"), FButtonEnum::Left))
				I::EngineClient->ClientCmd_Unrestricted(XS("toggleconsole"));
			if (FButton(XS("Fix materials"), FButtonEnum::Right | FButtonEnum::SameLine) && F::Materials.m_bLoaded)
				F::Materials.ReloadMaterials();

			if (!I::EngineClient->IsConnected())
			{
				if (FButton(XS("Unlock achievements"), FButtonEnum::Left))
					OpenPopup(XS("UnlockAchievements"));
				if (FButton(XS("Lock achievements"), FButtonEnum::Right | FButtonEnum::SameLine))
					OpenPopup(XS("LockAchievements"));

				if (FBeginPopupModal(XS("UnlockAchievements"), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
				{
					FText(XS("Do you really want to unlock all achievements?"));

					if (FButton(XS("Yes, unlock"), FButtonEnum::Left))
					{
				...
				else if (FBeginPopupModal(XS("LockAchievements"), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
				{
					FText(XS("Do you really want to lock all achievements?"));

					if (FButton(XS("Yes, lock"), FButtonEnum::Left))
					{
		...
		if (Vars::Debug::Options.Value && I::EngineClient->IsConnected())
		{
			if (Section(XS("##Debug"), -8))
			{
				if (FButton(XS("Restore lines"), FButtonEnum::Left))
				{
					for (auto& tLine : G::LineStorage)
						tLine.m_flTime = I::GlobalVars->curtime + 60.f;
				}
				if (FButton(XS("Restore paths"), FButtonEnum::Right | FButtonEnum::SameLine))
				{
					for (auto& tPath : G::PathStorage)
						tPath.m_flTime = I::GlobalVars->curtime + 60.f;
				}
				if (FButton(XS("Restore boxes"), FButtonEnum::Left))
				{
					for (auto& tBox : G::BoxStorage)
						tBox.m_flTime = I::GlobalVars->curtime + 60.f;
				}
				if (FButton(XS("Clear visuals"), FButtonEnum::Right | FButtonEnum::SameLine))
				{
					G::LineStorage.clear();
					G::PathStorage.clear();
					G::BoxStorage.clear();
		...
		if (Vars::Debug::Options.Value)
		{
			if (Section("Convar spoofer"))
			{
				...
				if (FButton("Send"))
				{
					...
					{
						SDK::Output("Convar", std::vformat("Sent {} as {}", std::make_format_args( sName, sValue)).c_str(), Vars::Menu::Theme::Accent.Value);
		...
		*/
#ifdef DEBUG_HOOKS
		if (Section(XS("Hooks"), 8))
		{
			int i = 0; for (auto& pBase : G::Vars)
			{
				if (std::string(pBase->Name()).find(XS("Vars::Hooks::")) == std::string::npos)
					continue;
		...
		uCurrHash != uStaticHash)
	{
		...
		for (auto& pBase : G::Vars)
		{
			...
			for (auto pSearch : vSearch)
			{
				...
				if (auto iFind = sSearch3.find(XS("Vars::")); iFind != std::string::npos)
					sSearch3 = sSearch3.replace(iFind, strlen(XS("Vars::")), "");
				if (auto iFind = sSearch3.find(XS("##")); iFind != std::string::npos)
					sSearch3 = sSearch3.replace(iFind, strlen(XS("##")), "");
	...
	int i = 0; for (auto pBase : vVars) // possibly implement tablelike visuals, do away with left right, just switch if current side is higher than other?
	{
		...
		else if (auto pVar = pBase->As<int>())
		{
			if (!pVar->m_vValues.empty()
				|| FNV1A::Hash32(pVar->Name()) == FNV1A::Hash32Const(XS("Vars::ESP::ActiveGroups")))
		...
		if (uSection != uLastSection)
		{
			if (uLastSection)
				EndSection();
			Section(std::vformat(XS("{}## {}"), std::make_format_args( pBase->Section(), pBase->Name())).c_str());
			i = 0;
		}
		...
		case WidgetEnum::FToggle:
		{
			...
			{
				if (FNV1A::Hash32(pVar->Name()) == FNV1A::Hash32Const(XS("Vars::Debug::Options")))
					uStaticHash = 0;
			}
			break;
		}
		...
		case WidgetEnum::FFSlider:
		{
			...
			{
			case FNV1A::Hash32Const(XS("Vars::Aimbot::Projectile::SplashRotateX")):
			case FNV1A::Hash32Const(XS("Vars::Aimbot::Projectile::SplashRotateY")):
		...
		case WidgetEnum::FKeybind:
		{
			...
			switch (FNV1A::Hash32(pVar->Name()))
			{
			case FNV1A::Hash32Const(XS("Vars::Menu::PrimaryKey")):
				vIgnore = { Vars::Menu::SecondaryKey[DEFAULT_BIND], VK_LBUTTON, VK_RBUTTON };
				break;
			case FNV1A::Hash32Const(XS("Vars::Menu::SecondaryKey")):
...
void CMenu::DrawBinds()
{
	...
	std::function<void(int)> getBinds = [&](int iParent)
		{
			for (int iBind = 0; iBind < F::Binds.m_vBinds.size(); iBind++)
			{
				...
				if (tBind.m_iVisibility == BindVisibilityEnum::Always || tBind.m_iVisibility == BindVisibilityEnum::WhileActive && tBind.m_bActive || m_bIsOpen)
				{
					...
					case BindEnum::ItemSlot:
						sType = XS("slot");
						sInfo = std::vformat(XS("{}"), std::make_format_args( tBind.m_iInfo + 1));
						break;
					case BindEnum::Misc:
						switch (tBind.m_iInfo)
						{
					...
					if (tBind.m_bNot && (tBind.m_iType != BindEnum::Key || tBind.m_iInfo == BindEnum::KeyEnum::Hold))
						sInfo = std::vformat(XS("not {}"), std::make_format_args( sInfo));
	...
	if (Begin(XS("Binds"), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing))
	{
		ImVec2 vWindowPos = GetWindowPos();
			...
			{	// buttons
				...
				else if (bDelete)
				{
					if (U::KeyHandler.Down(VK_SHIFT)) // allow user to quickly remove binds
						...
						OpenPopup(std::vformat(XS("DeleteBind{}"), std::make_format_args( iBind)).c_str());
				}

				if (FBeginPopupModal(std::vformat(XS("DeleteBind{}"), std::make_format_args( iBind)).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
				{
					FText(std::vformat(XS("Do you really want to delete '{}'{}?"), std::make_format_args( tBind.m_sName, F::Binds.HasChildren(iBind) ? XS(" and all of its children") : "")).c_str());
...
void CMenu::Render()
{
	...
	if (m_bIsOpen)
	{
		...
		AddDraggable(XS("Ticks"), Vars::Menu::TicksDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Ticks);
		AddDraggable(XS("Crit hack"), Vars::Menu::CritsDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::CritHack);
		AddDraggable(XS("Spectators"), Vars::Menu::SpectatorsDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Spectators);
		AddDraggable(XS("Ping"), Vars::Menu::PingDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Ping);
		AddDraggable(XS("Conditions"), Vars::Menu::ConditionsDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Conditions);
		AddDraggable(XS("Seed prediction"), Vars::Menu::SeedPredictionDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::SeedPrediction);
		AddResizableDraggable(XS("Camera"), Vars::Visuals::Simulation::ProjectileWindow, FGet(Vars::Visuals::Simulation::ProjectileCamera));
		...
		if (!vDisabled.empty())
		{
			IM_ASSERT_USER_ERROR(0, XS("Calling PopDisabled() too little times: stack overflow."));
			Disabled = false;
			vDisabled.clear();
		}
		if (!vTransparent.empty())
		{
			IM_ASSERT_USER_ERROR(0, XS("Calling PopTransparent() too little times: stack overflow."));
			Transparent = false;
			vTransparent.clear();
		}
	}
