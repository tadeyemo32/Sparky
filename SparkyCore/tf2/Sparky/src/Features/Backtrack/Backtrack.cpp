...
void CBacktrack::SendLerp()
{
	...
	if (m_flSentInterp != flTarget)
	{
		...
		{
			NET_SetConVar tConvar1 = { XS("cl_interp"), std::to_string(m_flSentInterp).c_str() };
			pNetChan->SendNetMsg(tConvar1);

			NET_SetConVar tConvar2 = { XS("cl_interp_ratio"), XS("1") };
			pNetChan->SendNetMsg(tConvar2);

			NET_SetConVar tConvar3 = { XS("cl_interpolate"), XS("1") };
...
void CBacktrack::MakeRecords()
{
	for (auto& pEntity : H::Entities.GetGroup(EntityEnum::PlayerAll))
	{
		...
		if (pLastRecord)
		{
			...
			static auto sv_lagcompensation_teleport_dist = H::ConVars.FindVar(XS("sv_lagcompensation_teleport_dist"));
...
void CBacktrack::Store()
{
	...
	static auto sv_maxunlag = H::ConVars.FindVar(XS("sv_maxunlag"));
	...
	auto Set = [&]()
		{
			...
			static auto host_timescale = H::ConVars.FindVar(XS("host_timescale"));
