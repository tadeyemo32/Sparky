...
MAKE_SIGNATURE(CSoundEmitterSystem_EmitSound, XS("client.dll"), XS("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 41 56 48 81 EC ? ? ? ? 49 8B D9"), 0x0);
//MAKE_SIGNATURE(S_StartDynamicSound, "engine.dll", "4C 8B DC 57 48 81 EC", 0x0);
MAKE_SIGNATURE(S_StartSound, XS("engine.dll"), XS("40 53 48 83 EC ? 48 83 79 ? ? 48 8B D9 75 ? 33 C0"), 0x0);
MAKE_SIGNATURE(CBaseEntity_EmitSound, XS("client.dll"), XS("48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 ? 48 81 EC ? ? ? ? 48 8B 3D"), 0x0);
...
const static std::vector<const char*> s_vFootsteps = { XS("footstep"), XS("flesh_impact_hard"), XS("body_medium_impact_soft"), XS("glass_sheet_step"), XS("rubber_tire_impact_soft"), XS("plastic_box_impact_soft"), XS("plastic_barrel_impact_soft"), XS("cardboard_box_impact_soft"), XS("ceiling_tile_step") };
const static std::vector<const char*> s_vNoisemaker = { XS("items\\halloween"), XS("items\\football_manager"), XS("items\\japan_fundraiser"), XS("items\\samurai\\tf_samurai_noisemaker"), XS("items\\summer"), XS("misc\\happy_birthday_tf"), XS("misc\\jingle_bells") };
const static std::vector<const char*> s_vFryingPan = { XS("pan_") };
const static std::vector<const char*> s_vWater = { XS("ambient_mp3\\water\\water_splash"), XS("slosh"), XS("wade") };
	...
	void* rcx, const char* soundname, float soundtime, float* duration)
{
	...
	if (soundname)
	{
		...
		{
		case FNV1A::Hash32Const(XS("BumperCar.Jump")):
		case FNV1A::Hash32Const(XS("BumperCar.JumpLand")):
		case FNV1A::Hash32Const(XS("BumperCar.Bump")):
		case FNV1A::Hash32Const(XS("BumperCar.BumpHard")):
