...
MAKE_SIGNATURE(FX_FireBullets, XS("client.dll"), XS("48 89 5C 24 ? 48 89 74 24 ? 4C 89 4C 24 ? 55"), 0x0);
MAKE_SIGNATURE(CTFWeaponBaseGun_FireBullet_FireBullets_Call, XS("client.dll"), XS("0F 28 7C 24 ? 4C 8D 9C 24 ? ? ? ? 49 8B 5B ? 49 8B 6B ? 49 8B 73 ? 41 0F 28 73 ? 49 8B E3"), 0x0);
...
MAKE_SIGNATURE(FX_FireBullets_Server, XS("server.dll"), XS("48 89 5C 24 ? 4C 89 4C 24 ? 55 56 41 54"), 0x0);
MAKE_SIGNATURE(CBasePlayer_ProcessUsercmds, XS("server.dll"), XS("40 53 55 56 57 41 54 48 83 EC ? 4C 89 6C 24"), 0x0);
	...
	CTFWeaponBase* pWpn, int iPlayer, const Vec3& vecOrigin, const Vec3& vecAngles, int iWeapon, int iMode, int iSeed, float flSpread, float flDamage, bool bCritical)
{
	...
	if (Vars::Aimbot::General::NoSpread.Value)
		SDK::Output(XS("FX_FireBullets"), std::vformat(XS("{}"), std::make_format_args( iSeed)).c_str(), { 0, 255, 0 });
	return CALL_ORIGINAL(pWpn, iPlayer, vecOrigin, vecAngles, iWeapon, iMode, iSeed, flSpread, flDamage, bCritical);
}
	...
	void* rcx, CUserCmd* cmds, int numcmds, int totalcmds, int dropped_packets, bool paused)
{
	...
	if (bInAttack)
	{
		...
		SDK::Output(XS("ProcessUsercmds"), std::vformat(XS("{}: {}"), std::make_format_args( dFloatTime, iSeed)).c_str(), { 0, 255, 0, 100 });
	}
