#include "../SDK/SDK.h"

enum EHoliday
{
	kHoliday_None = 0,
	kHoliday_TFBirthday,
	kHoliday_Halloween,
	kHoliday_Christmas,
	kHoliday_CommunityUpdate,
	kHoliday_EOTL,
	kHoliday_Valentines,
	kHoliday_MeetThePyro,
	kHoliday_FullMoon,
	kHoliday_HalloweenOrFullMoon,
	kHoliday_HalloweenOrFullMoonOrValentines,
	kHoliday_AprilFools,
	kHolidayCount,
};

MAKE_SIGNATURE(TF_IsHolidayActive, XS("client.dll"), XS("48 83 EC ? 48 8B 05 ? ? ? ? 44 8B C9"), 0x0);
MAKE_SIGNATURE(CTFPlayer_FireEvent_IsHolidayActive_Call, XS("client.dll"), XS("84 C0 74 ? 45 33 C9 C6 44 24 ? ? 4C 8B C6"), 0x0);

MAKE_HOOK(TF_IsHolidayActive, S::TF_IsHolidayActive(), bool,
	int eHoliday)
{
	DEBUG_RETURN(TF_IsHolidayActive, eHoliday);

	const auto dwRetAddr = uintptr_t(_ReturnAddress());
	const auto dwDesired = S::CTFPlayer_FireEvent_IsHolidayActive_Call();

	if (dwRetAddr == dwDesired)
		return true;
	
	return CALL_ORIGINAL(eHoliday);
}