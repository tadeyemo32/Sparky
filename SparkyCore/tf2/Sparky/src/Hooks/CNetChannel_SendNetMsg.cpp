...
MAKE_SIGNATURE(CNetChannel_SendNetMsg, XS("engine.dll"), XS("48 89 5C 24 ? 48 89 74 24 ? 48 89 7C 24 ? 41 56 48 83 EC ? 48 8B F1 45 0F B6 F1"), 0x0);
	...
	CNetChannel* pNetChan, INetMessage& msg, bool bForceReliable, bool bVoice)
{
	...
	case net_SetConVar:
	{
		...
		for (int i = 0; i < pMsg->m_ConVars.Count(); i++)
		{
			...
			{
			case FNV1A::Hash32Const(XS("cl_interp")):
			...
			case FNV1A::Hash32Const(XS("cl_updaterate")):
				...
				break;
			case FNV1A::Hash32Const(XS("cl_interp_ratio")):
			case FNV1A::Hash32Const(XS("cl_interpolate")):
				strncpy_s(localCvar->Value, XS("1"), MAX_OSPATH);
			}
			...
			if (Vars::Debug::Logging.Value)
			{
				...
				case FNV1A::Hash32Const(XS("cl_cmdrate")):
				case FNV1A::Hash32Const(XS("cl_updaterate")):
					SDK::Output(XS("SendNetMsg"), XSFMT(XS("{}: {}"), localCvar->Name, localCvar->Value).c_str(), { 100, 0, 255 });
				}
			}
		}
		break;
		...
		if (Vars::Misc::Game::AntiCheatCompatibility.Value)
		{
			...
			case FNV1A::Hash32Const(XS("cl_updaterate")):
				if (F::Misc.m_iWishUpdaterate != -1)
					...
					sValue = pConVar->GetString();
				break;
			case FNV1A::Hash32Const(XS("mat_dxlevel")):
				sValue = pConVar->GetString();
				break;
			default:
			...
			SDK::Output(XS("Convar spoof"), msg.ToString(), Vars::Menu::Theme::Accent.Value, Vars::Debug::Logging.Value);
		}
		break;
	case clc_Move:
	{
		...
		if (!F::Ticks.m_bSpeedhack)
		{
			...
			{
				SDK::Output(XS("clc_Move"), XSFMT(XS("{:d} sent <{:d} | {:d}>, max was {:d}."), iCmdCount + 3, pMsg->m_nNewCommands, pMsg->m_nBackupCommands, iAllowedNewCommands).c_str(), { 255, 0, 0, 255 });
