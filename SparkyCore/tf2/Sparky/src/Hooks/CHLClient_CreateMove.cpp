...
MAKE_SIGNATURE(IHasGenericMeter_GetMeterMultiplier, XS("client.dll"), XS("F3 0F 10 81 ? ? ? ? C3 CC CC CC CC CC CC CC 48 85 D2"), 0x0);
	...
	if (bCanAttack)
	{
		...
		case TF_WEAPON_FLAME_BALL:
			if (G::CanPrimaryAttack)
			{
				...
				float flRate = SDK::AttribHookValue(1.f, XS("item_meter_charge_rate"), pWeapon) - 1;
				float flMult = SDK::AttribHookValue(1.f, XS("mult_item_meter_charge_rate"), pWeapon);
...
static inline void AntiCheatCompatibility(CUserCmd* pCmd, bool* pSendPacket)
{
	...
	static CmdHistory_t vHistory[5];
	static int iHistoryCount = 0;

	for (int i = std::min(iHistoryCount, 4); i > 0; --i)
		vHistory[i] = vHistory[i - 1];

	vHistory[0].m_vAngle = pCmd->viewangles;
	vHistory[0].m_bAttack1 = pCmd->buttons & IN_ATTACK;
	vHistory[0].m_bAttack2 = pCmd->buttons & IN_ATTACK2;
	vHistory[0].m_bSendingPacket = *pSendPacket;
