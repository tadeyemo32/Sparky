...
MAKE_SIGNATURE(CTFPlayer_FireBullet, XS("client.dll"), XS("48 89 74 24 ? 55 57 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? F3 41 0F 10 58"), 0x0);
	...
	void* rcx, CBaseCombatWeapon* pWeapon, const FireBulletsInfo_t& info, bool bDoEffects, int nDamageType, int nCustomDamageType)
{
	...
	if (uHash == FNV1A::Hash32Const(XS("Default")))
		return CALL_ORIGINAL(rcx, pWeapon, info, bDoEffects, nDamageType, nCustomDamageType);
	else if (uHash == FNV1A::Hash32Const(XS("None")))
		return;
	...
	int iAttachment = pWeapon->LookupAttachment(XS("muzzle"));
	pWeapon->GetAttachment(iAttachment, trace.startpos);

	switch (uHash)
	{
	case FNV1A::Hash32Const(XS("Big nasty")):
		H::Particles.ParticleTracer(iTeam == TF_TEAM_RED ? XS("bullet_bignasty_tracer01_blue") : XS("bullet_bignasty_tracer01_red"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Distortion trail")):
		H::Particles.ParticleTracer(XS("tfc_sniper_distortion_trail"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Machina")):
		H::Particles.ParticleTracer(iTeam == TF_TEAM_RED ? XS("dxhr_sniper_rail_red") : XS("dxhr_sniper_rail_blue"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Sniper rail")):
		H::Particles.ParticleTracer(XS("dxhr_sniper_rail"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Short circuit")):
		H::Particles.ParticleTracer(iTeam == TF_TEAM_RED ? XS("dxhr_lightningball_hit_zap_red") : XS("dxhr_lightningball_hit_zap_blue"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("C.A.P.P.E.R")):
		H::Particles.ParticleTracer(iTeam == TF_TEAM_RED ? XS("bullet_tracer_raygun_red") : XS("bullet_tracer_raygun_blue"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Merasmus ZAP")):
		H::Particles.ParticleTracer(XS("merasmus_zap"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Merasmus ZAP 2")):
		H::Particles.ParticleTracer(XS("merasmus_zap_beam02"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Black ink")):
		H::Particles.ParticleTracer(XS("merasmus_zap_beam01"), trace.startpos, trace.endpos, iIndex, iAttachment, true);
		break;
	case FNV1A::Hash32Const(XS("Line")):
	case FNV1A::Hash32Const(XS("Line ignore Z")):
	{
		...
		if (uHash == FNV1A::Hash32Const(XS("Line")))
	...
	case FNV1A::Hash32Const(XS("Beam")):
	{
		BeamInfo_t beamInfo;
		beamInfo.m_nType = 0;
		beamInfo.m_pszModelName = !Vars::Visuals::Beams::Model.Value.empty() ? Vars::Visuals::Beams::Model.Value.c_str() : XS("sprites/physbeam.vmt");
