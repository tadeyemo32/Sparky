...
MAKE_SIGNATURE(CPlayerResource_IsFakePlayer, XS("client.dll"), XS("48 89 5C 24 ? 57 48 81 EC ? ? ? ? 48 8B 01 48 8B F9 48 63 DA"), 0x0);
MAKE_SIGNATURE(VGuiMenuBuilder_AddMenuItem, XS("client.dll"), XS("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC ? 48 8B EA 49 8B F9 48 8B 51 ? 49 8B F0 48 8B D9 48 85 D2 74 ? 49 8B C9 E8 ? ? ? ? 85 C0 74 ? 48 8B 0B 48 8B 01 FF 90 ? ? ? ? 48 8B 0B 4C 8B C6 4C 8B 4B ? 48 8B D5 48 89 7B ? 48 C7 44 24 ? ? ? ? ? 48 8B 01 FF 90 ? ? ? ? 48 8B 13 3B 82 ? ? ? ? 73 ? 3B 82 ? ? ? ? 7F ? 48 8B 92 ? ? ? ? 8B C8 48 03 C9 39 44 CA ? 75 ? 39 44 CA ? 75 ? 48 8B 04 CA EB ? 33 C0 48 8B 5C 24 ? 48 8B 6C 24 ? 48 8B 74 24 ? 48 83 C4 ? 5F C3 CC CC CC CC 48 89 5C 24"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_OnCommand, XS("client.dll"), XS("48 89 5C 24 ? 57 48 83 EC ? 48 8B DA 48 8B F9 48 8B CB 48 8D 15 ? ? ? ? E8 ? ? ? ? 48 85 C0 74 ? 48 8B 0D"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_OnScoreBoardMouseRightRelease_IsFakePlayer_Call, XS("client.dll"), XS("48 8B 0D ? ? ? ? 41 8B D6 48 81 C1 ? ? ? ? 44 0F B6 E8"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_OnScoreBoardMouseRightRelease_AddMenuItem_CallProfile, XS("client.dll"), XS("48 8B 0D ? ? ? ? 4C 8D 85 ? ? ? ? 48 8D 95 ? ? ? ? 48 8B 01 FF 90 ? ? ? ? 44 8B 85"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_OnScoreBoardMouseRightRelease_AddMenuItem_CallSpectate, XS("client.dll"), XS("45 84 ED 4C 8B AC 24 ? ? ? ? 0F 85"), 0x0);
	...
	void* rcx, const char* pszButtonText, const char* pszCommand, const char* pszCategoryName)
{
	...
	if (dwRetAddr == dwDesired1 && Vars::Visuals::UI::ScoreboardUtility.Value)
	{
		if (auto pResource = H::Entities.GetResource(); pResource && pResource->m_bValid(s_iPlayerIndex))
		{
			...
			CALL_ORIGINAL(rcx, XSFMT(XS("Tags for {}"), s_sPlayerName).c_str(), XS("listtags"), XS("tags"));
			for (auto it = F::PlayerUtils.m_vTags.begin(); it != F::PlayerUtils.m_vTags.end(); it++)
			{
				...
				CALL_ORIGINAL(rcx, XSFMT(XS("{} {}"), bHasTag ? XS("Remove") : XS("Add"), tTag.m_sName).c_str(), XSFMT(XS("modifytag{}"), iID).c_str(), XS("tags"));
			}
	...
	void* rcx, const char* command)
{
	...
	case FNV1A::Hash32Const(XS("history")):
		I::SteamFriends->ActivateGameOverlayToWebPage(XSFMT(XS("https://steamhistory.net/id/{}"), CSteamID(s_uAccountID, k_EUniversePublic, k_EAccountTypeIndividual).ConvertToUint64()).c_str());
		break;
	case FNV1A::Hash32Const(XS("listtags")):
		F::Output.TagsOnJoin(s_sPlayerName, s_uAccountID);
		break;
	case FNV1A::Hash32Const(XS("specplayer")):
		...
		[[fallthrough]];
	default:
		if (strstr(command, XS("modifytag")))
		{
			try
			{
				std::string sTag = command;
				sTag = sTag.replace(0, strlen(XS("modifytag")), "");
