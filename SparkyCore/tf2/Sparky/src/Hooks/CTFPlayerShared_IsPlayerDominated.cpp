...
MAKE_SIGNATURE(CTFPlayerShared_IsPlayerDominated, XS("client.dll"), XS("48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC ? 48 63 F2 48 8B D9 E8"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_UpdatePlayerList_IsPlayerDominated_Call, XS("client.dll"), XS("84 C0 74 ? 45 84 FF 74"), 0x0);
MAKE_SIGNATURE(KeyValues_SetInt, XS("client.dll"), XS("40 53 48 83 EC ? 41 8B D8 41 B0"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_UpdatePlayerList_SetInt_Call, XS("client.dll"), XS("48 8B 05 ? ? ? ? 83 78 ? ? 0F 84 ? ? ? ? 48 8B 0D ? ? ? ? 8B D7"), 0x0);
MAKE_SIGNATURE(CTFClientScoreBoardDialog_UpdatePlayerList_Jump, XS("client.dll"), XS("8B E8 E8 ? ? ? ? 3B C7"), 0x0);
MAKE_SIGNATURE(CClientScoreBoardDialog_NeedsUpdate, XS("client.dll"), XS("48 8B 05 ? ? ? ? F3 0F 10 41 ? 0F 2F 40 ? 0F 92 C0"), 0x0);
	...
	void* rcx, const char* keyName, int value)
{
	...
	if (dwRetAddr == dwDesired && Vars::Visuals::UI::RevealScoreboard.Value && !SDK::CleanScreenshot() && keyName && FNV1A::Hash32(keyName) == FNV1A::Hash32Const(XS("nemesis")))
		*static_cast<uintptr_t*>(_AddressOfReturnAddress()) = dwJump;
}
