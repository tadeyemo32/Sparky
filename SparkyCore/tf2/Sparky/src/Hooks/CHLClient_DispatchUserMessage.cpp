	...
	void* rcx, UserMessageType type, bf_read& msgData)
{
	...
	case TextMsg:
	{
		...
		if (!sMsg.empty())
		{
...
#ifdef DEBUG_VISUALS
			if (sMsg.find(XS("[Box] ")) == 0)
			{
				try
				{
					sMsg.replace(0, strlen(XS("[Box] ")), "");
					std::vector<std::string> vValues = {};
					boost::split(vValues, sMsg, boost::is_any_of(XS(" ")));
					if (vValues.size() != 17)
						return true;
			...
			if (sMsg.find(XS("[Line] ")) == 0)
			{
				try
				{
					sMsg.replace(0, strlen(XS("[Line] ")), "");
					std::vector<std::string> vValues = {};
					boost::split(vValues, sMsg, boost::is_any_of(XS(" ")));
					if (vValues.size() != 11)
						return true;
			...
			if (Vars::Misc::Automation::AntiAutobalance.Value && FNV1A::Hash32(sMsg.c_str()) == FNV1A::Hash32Const(XS("#TF_Autobalance_TeamChangePending")))
	...
	case VGUIMenu:
		if (Vars::Visuals::Removals::MOTD.Value && bufData
			&& FNV1A::Hash32(bufData) == FNV1A::Hash32Const(XS("info")))
		{
			I::EngineClient->ClientCmd_Unrestricted(XS("closedwelcomemenu"));
			return true;
		}
		break;
