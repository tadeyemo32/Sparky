...
MAKE_SIGNATURE(CAttributeManager_AttribHookInt, XS("client.dll"), XS("4C 8B DC 49 89 5B ? 49 89 6B ? 49 89 73 ? 57 41 54 41 55 41 56 41 57 48 83 EC ? 48 8B 3D ? ? ? ? 4C 8D 35"), 0x0);
MAKE_SIGNATURE(CTFPlayer_FireEvent_AttribHookValue_Call, XS("client.dll"), XS("8B F8 83 BE"), 0x0);
	...
	int value, const char* name, void* econent, void* buffer, bool isGlobalConstString)
{
	...
	if (dwRetAddr == dwDesired && Vars::Visuals::Effects::SpellFootsteps.Value
		&& econent == H::Entities.GetLocal() && FNV1A::Hash32(name) == FNV1A::Hash32Const(XS("halloween_footstep_type")))
	{
