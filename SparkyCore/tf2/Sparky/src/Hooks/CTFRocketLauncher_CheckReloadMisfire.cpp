...
MAKE_SIGNATURE(CTFRocketLauncher_CheckReloadMisfire, XS("client.dll"), XS("48 83 EC ? E8 ? ? ? ? 32 C0"), 0x0);
MAKE_SIGNATURE(CTFRocketLauncher_FireProjectile, XS("client.dll"), XS("48 89 74 24 ? 57 48 81 EC ? ? ? ? 48 8B FA 48 89 9C 24"), 0x0);
	...
	void* rcx)
{
	...
	if (!SDK::AttribHookValue(0, XS("can_overload"), pWeapon))
		return false;
