...
MAKE_SIGNATURE(CBaseHudChatLine_InsertAndColorizeText, XS("client.dll"), XS("44 89 44 24 ? 55 53 56 57"), 0x0);
	...
	void* rcx, wchar_t* buf, int clientIndex)
{
	...
	if (const char* sReplace = F::PlayerUtils.GetPlayerName(clientIndex, nullptr, &iType))
	{
		if (iFind != std::string::npos)
			sMessage = sMessage.replace(std::max(int(iFind) - 1, 0), strlen(sName) + 1, XSFMT(XS("\x3{}\x1"), sReplace));
		sName = sReplace;
	}
	...
	if (Vars::Visuals::UI::ChatTags.Value && !(iType & NameTypeEnum::Privacy))
	{
		...
		if (!sTag.empty())
		{
			if (iFind != std::string::npos && iType == NameTypeEnum::None)
				sMessage = sMessage.replace(std::max(int(iFind) - 1, 0), strlen(sName) + 1, XSFMT(XS("\x3{}\x1"), sName));
			sMessage.insert(0, XSFMT(XS("{}[{}] "), cColor, sTag));
		}
	}
