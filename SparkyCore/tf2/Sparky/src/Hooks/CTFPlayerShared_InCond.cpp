...
MAKE_SIGNATURE(CTFPlayerShared_InCond, XS("client.dll"), XS("48 89 5C 24 ? 57 48 83 EC ? 8B DA 48 8B F9 83 FA ? 7D"), 0x0);
MAKE_SIGNATURE(CTFPlayer_ShouldDraw_InCond_Call, XS("client.dll"), XS("84 C0 74 ? 32 C0 48 8B 74 24"), 0x0);
MAKE_SIGNATURE(CTFWearable_ShouldDraw_InCond_Call, XS("client.dll"), XS("84 C0 0F 85 ? ? ? ? 41 BF"), 0x0);
MAKE_SIGNATURE(CHudScope_ShouldDraw_InCond_Call, XS("client.dll"), XS("84 C0 74 ? 48 8B CB E8 ? ? ? ? 48 85 C0 74 ? 48 8B CB E8 ? ? ? ? 48 8B C8 48 8B 10 FF 92 ? ? ? ? 83 F8 ? 0F 94 C0"), 0x0);
MAKE_SIGNATURE(CTFPlayer_CreateMove_InCondTaunt_Call, XS("client.dll"), XS("84 C0 75 ? BA ? ? ? ? 48 8D 8E ? ? ? ? E8 ? ? ? ? 84 C0 75 ? 45 32 FF"), 0x0);
MAKE_SIGNATURE(CTFPlayer_CreateMove_InCondKart_Call, XS("client.dll"), XS("84 C0 74 ? 4C 8B C3"), 0x0);
MAKE_SIGNATURE(CTFInput_ApplyMouse_InCond_Call, XS("client.dll"), XS("84 C0 74 ? F3 0F 10 9B"), 0x0);
	...
	void* rcx, ETFCond nCond)
{
	...
	auto GetOuter = [&rcx]() -> CBaseEntity*
		{
			static const auto iShared = U::NetVars.GetNetVar(XS("CTFPlayer"), XS("m_Shared"));
			static const auto iBombHeadStage = U::NetVars.GetNetVar(XS("CTFPlayer"), XS("m_nHalloweenBombHeadStage"));
