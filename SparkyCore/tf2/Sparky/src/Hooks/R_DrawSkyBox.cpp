...
MAKE_SIGNATURE(R_DrawSkyBox, XS("engine.dll"), XS("48 8B C4 55 53 41 54 41 55"), 0x0);
	...
	float zFar, int nDrawFlags)
{
	...
	if (FNV1A::Hash32(Vars::Visuals::World::SkyboxChanger.Value.c_str()) == FNV1A::Hash32Const(XS("Off")) || SDK::CleanScreenshot())
		return CALL_ORIGINAL(zFar, nDrawFlags);

	static auto sv_skyname = H::ConVars.FindVar(XS("sv_skyname"));
