...
MAKE_SIGNATURE(CAttributeManager_AttribHookFloat, XS("client.dll"), XS("4C 8B DC 49 89 5B ? 49 89 6B ? 56 57 41 54 41 56 41 57 48 83 EC ? 48 8B 3D ? ? ? ? 4C 8D 35"), 0x0);
...
static BOOL CALLBACK TeamFortressWindow(HWND hWindow, LPARAM lParam)
{
	...
	{
	case FNV1A::Hash32Const(XS("Team Fortress 2 - Direct3D 9 - 64 Bit")):
	case FNV1A::Hash32Const(XS("Team Fortress 2 - Vulkan - 64 Bit")):
		break;
	default:
		return TRUE;
	}
	...
	const char* sLeft, const char* sRight)
{
	if (sLog)
	{
		...
		{
			I::CVar->ConsoleColorPrintf(tColor, XS("%s%s%s "), sLeft, sFunction, sRight);
			I::CVar->ConsoleColorPrintf({}, XS("%s\n"), sLog);
		}
		if (iTo & OUTPUT_DEBUG)
			OutputDebugString(std::vformat(XS("{}{}{} {}\n"), std::make_format_args( sLeft, sFunction, sRight, sLog)).c_str());
		if (iTo & OUTPUT_TOAST)
			F::Notifications.Add(sLog, tColor);
		if (iTo & OUTPUT_MENU)
			F::Menu.AddOutput(std::vformat(XS("{}{}{}"), std::make_format_args( sLeft, sFunction, sRight)).c_str(), sLog, tColor);
		if (iTo & OUTPUT_CHAT)
			I::ClientModeShared->m_pChatElement->ChatPrintf(0, std::vformat(XS("{}{}{}{}\x1 {}"), std::make_format_args( tColor.ToHex(), sLeft, sFunction, sRight, sLog)).c_str());
		if (iTo & OUTPUT_PARTY)
	...
	else
	{
		...
		if (iTo & OUTPUT_DEBUG)
			OutputDebugString(std::vformat(XS("{}\n"), std::make_format_args( sFunction)).c_str());
		if (iTo & OUTPUT_TOAST)
			F::Notifications.Add(sFunction, tColor);
		if (iTo & OUTPUT_MENU)
			F::Menu.AddOutput("", sFunction, tColor);
		if (iTo & OUTPUT_CHAT)
			I::ClientModeShared->m_pChatElement->ChatPrintf(0, std::vformat(XS("{}{}\x1"), std::make_format_args( tColor.ToHex(), sFunction)).c_str());
		if (iTo & OUTPUT_PARTY)
...
std::string SDK::GetDate()
{
	...
	char buffer[16]; strftime(buffer, sizeof(buffer), XS("%b %e %Y"), &timeinfo);
	return buffer;
}
...
std::string SDK::GetTime()
{
	time_t tTime = time(nullptr);
	tm timeinfo; localtime_s(&timeinfo, &tTime);
	char buffer[16]; strftime(buffer, sizeof(buffer), XS("%T"), &timeinfo);
	return buffer;
}
...
{
	static auto Plat_FloatTime = U::Memory.GetModuleExport<double(*)()>(XS("tier0.dll"), XS("Plat_FloatTime"));
...
{
	static auto RandomSeed = U::Memory.GetModuleExport<void(*)(uint32_t)>(XS("vstdlib.dll"), XS("RandomSeed"));
	RandomSeed(iSeed);
}

int SDK::RandomInt(int iMinVal, int iMaxVal)
{
	static auto RandomInt = U::Memory.GetModuleExport<int(*)(int, int)>(XS("vstdlib.dll"), XS("RandomInt"));
	return RandomInt(iMinVal, iMaxVal);
}

float SDK::RandomFloat(float flMinVal, float flMaxVal)
{
	static auto RandomFloat = U::Memory.GetModuleExport<float(*)(float, float)>(XS("vstdlib.dll"), XS("RandomFloat"));
...
const char* SDK::GetClassByIndex(const int nClass, bool bLower)
{
	static const char* szClassesUpper[] = {
		XS("Unknown"), XS("Scout"), XS("Sniper"), XS("Soldier"), XS("Demoman"), XS("Medic"), XS("Heavy"), XS("Pyro"), XS("Spy"), XS("Engineer")
	};
...
int SDK::IsAttacking(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, const CUserCmd* pCmd, bool bTickBase)
{
	...
	case TF_WEAPON_GRENADE_STICKY_BALL:
	{
		...
		const float flAmount = Math::RemapVal(flCharge, 0.f, SDK::AttribHookValue(4.f, XS("stickybomb_charge_rate"), pWeapon), 0.f, 1.f);
		return !(pCmd->buttons & IN_ATTACK) && flAmount > 0.f || flAmount == 1.f;
	}
	case TF_WEAPON_CANNON:
	{
		float flMortar = SDK::AttribHookValue(0.f, XS("grenade_launcher_mortar_mode"), pWeapon);
		if (!flMortar)
		...
		const float flAmount = Math::RemapVal(flCharge, 0.f, SDK::AttribHookValue(0.f, XS("grenade_launcher_mortar_mode"), pWeapon), 0.f, 1.f);
		return !(pCmd->buttons & IN_ATTACK) && flAmount > 0.f || flAmount == 1.f;
	}
	...
	case TF_WEAPON_LUNCHBOX:
		...
		return false;
	case TF_WEAPON_FLAMETHROWER:
		if (!SDK::AttribHookValue(0, XS("set_charged_airblast"), pWeapon)
...
{
	static auto cl_flipviewmodels = H::ConVars.FindVar(XS("cl_flipviewmodels"));
