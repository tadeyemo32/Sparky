...
struct Color_t
{
	...
	{
		return std::vformat(XS("\x7{:02x}{:02x}{:02x}"), std::make_format_args( r, g, b));
	}

	inline std::string ToHexA() const
	{
		return std::vformat(XS("\x8{:02x}{:02x}{:02x}{:02x}"), std::make_format_args( r, g, b, a));
	}
...
struct Chams_t
{
	std::vector<std::pair<std::string, Color_t>> Visible = { { XS("Original"), Color_t() } };
	...
	{
		return Visible != std::vector<std::pair<std::string, Color_t>>{ { XS("Original"), Color_t() } } || !bVisibleOnly && !Occluded.empty();
	}
};
