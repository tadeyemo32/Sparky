...
public:
	...
	const char* ToString()
	{
		...
		{
		case DPT_Int:
			sprintf_s(text, sizeof(text), XS("%i"), m_Int);
			break;
		case DPT_Float:
			sprintf_s(text, sizeof(text), XS("%.3f"), m_Float);
			break;
		case DPT_Vector:
			sprintf_s(text, sizeof(text), XS("(%.3f,%.3f,%.3f)"),
				m_Vector[0], m_Vector[1], m_Vector[2]);
			break;
#if 0 // We can't ship this since it changes the size of DTVariant to be 20 bytes instead of 16 and that breaks MODs!!!
		case DPT_Quaternion:
			sprintf_s(text, sizeof(text), XS("(%.3f,%.3f,%.3f %.3f)"),
		...
		case DPT_String:
			...
			break;
		case DPT_Array:
			sprintf_s(text, sizeof(text), XS("Array"));
			break;
		case DPT_DataTable:
			sprintf_s(text, sizeof(text), XS("DataTable"));
			break;
		default:
			sprintf_s(text, sizeof(text), XS("DVariant type %i unknown"), m_Type);
			break;
		}
