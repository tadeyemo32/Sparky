...
#define BEGIN_SEND_TABLE(className, tableName) \
	BEGIN_SEND_TABLE_NOBASE(className, tableName) \
		SendPropDataTable(XS("baseclass"), 0, className::BaseClass::m_pClassSendTable, SendProxy_DataTableToDataTable),
...
#define BEGIN_SEND_TABLE_NOBASE(className, tableName) \
	...
	{ \
		...
		static SendProp g_SendProps[] = { \
			SendPropInt(XS("should_never_see_this"), 0, sizeof(int)),		// It adds a dummy property at the start so you can define "empty" SendTables.
