...
#define BEGIN_RECV_TABLE(className, tableName) \
	BEGIN_RECV_TABLE_NOBASE(className, tableName) \
		RecvPropDataTable(XS("baseclass"), 0, 0, className::BaseClass::m_pClassRecvTable, DataTableRecvProxy_StaticDataTable),
...
#define BEGIN_RECV_TABLE_NOBASE(className, tableName) \
	...
	{ \
		...
		static RecvProp RecvProps[] = { \
			RecvPropInt(XS("should_never_see_this"), 0, sizeof(int)),		// It adds a dummy property at the start so you can define "empty" SendTables.
