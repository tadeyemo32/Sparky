...
const char* CLC_Move::ToString(void) const { return XS("CLC_Move"); }
...
{
    return std::vformat(XS("{}: {} cvars, \"{}\"=\"{}\""), std::make_format_args( GetName(), m_ConVars.Count(), m_ConVars[0].Name, m_ConVars[0].Value)).c_str();
}
...
{
    return std::vformat(XS("net_SignonState: state {}, count {}"), std::make_format_args( m_nSignonState, m_nSpawnCount)).c_str();
}
...
{
    return std::vformat(XS("{}: tick {}"), std::make_format_args( GetName(), m_nTick)).c_str();
}

const char* CLC_VoiceData::ToString(void) const
{
    return std::vformat(XS("{}: {} bytes"), std::make_format_args( GetName(), static_cast<int>(m_nLength * 0.125f))).c_str();
}
...
{
    return std::vformat(XS("{}: tick {}"), std::make_format_args( GetName(), m_nBaselineTick)).c_str();
}