...
MAKE_SIGNATURE(KeyValues_LoadFromBuffer, XS("engine.dll"), XS("4C 89 4C 24 ? 48 89 4C 24 ? 55 56"), 0x0);
MAKE_SIGNATURE(KeyValues_Initialize, XS("engine.dll"), XS("40 53 48 83 EC ? 48 8B D9 C7 01"), 0x0);
MAKE_SIGNATURE(KeyValues_GetSymbolForStringClassic, XS("engine.dll"), XS("48 89 5C 24 ? 57 48 83 EC ? 0F B6 DA 48 8B F9"), 0x0);
MAKE_SIGNATURE(KeyValues_GetStringForSymbolClassic, XS("engine.dll"), XS("40 53 48 83 EC ? 8B D9 FF 15"), 0x0);
MAKE_SIGNATURE(KeyValues_FindKey, XS("client.dll"), XS("48 8B C4 53 57 41 56"), 0x0);
...
const char* KeyValues::GetString(const char* keyName, const char* defaultValue)
{
	...
	if (dat)
	{
		...
		{
		case TYPE_FLOAT:
			snprintf(buf, sizeof(buf), XS("%f"), dat->m_flValue);
			SetString(keyName, buf);
			break;
		case TYPE_PTR:
			snprintf(buf, sizeof(buf), XS("%lld"), (int64_t)(size_t)dat->m_pValue);
			SetString(keyName, buf);
			break;
		case TYPE_INT:
			snprintf(buf, sizeof(buf), XS("%d"), dat->m_iValue);
			SetString(keyName, buf);
			break;
		case TYPE_UINT64:
			snprintf(buf, sizeof(buf), XS("%lld"), *((uint64_t*)(dat->m_sValue)));
			SetString(keyName, buf);
			break;
		case TYPE_WSTRING:
		{
...
Color_t KeyValues::GetColor(const char* keyName)
{
	...
	if (dat)
	{
		...
		else if (dat->m_iDataType == TYPE_STRING)
		{
			...
			sscanf_s(dat->m_sValue, XS("%f %f %f %f"), &a, &b, &c, &d);
