...
inline const char *servernetadr_t::ToString( uint32 unIP, uint16 usPort ) const
{
	...
	snprintf(s[nBuf], sizeof( s[nBuf] ), XS("%u.%u.%u.%u:%i"), (int)(ipByte[0]), (int)(ipByte[1]), (int)(ipByte[2]), (int)(ipByte[3]), usPort );
#else
	snprintf(s[nBuf], sizeof( s[nBuf] ), XS("%u.%u.%u.%u:%i"), (int)(ipByte[3]), (int)(ipByte[2]), (int)(ipByte[1]), (int)(ipByte[0]), usPort );
#endif
