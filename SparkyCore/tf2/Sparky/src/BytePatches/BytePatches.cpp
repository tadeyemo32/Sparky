...
bool BytePatch::Initialize()
{
	...
	if (!m_pAddress)
	{
		U::Core.AppendFailText(XSFMT(XS("BytePatch::Initialize() failed to initialize:\n  {}\n  {}"), m_sModule, m_sSignature).c_str());
		return false;
	}
...
{
	m_vPatches = {
		BytePatch(XS("engine.dll"), XS("0F 82 ? ? ? ? 4A 63 84 2F"), 0x0, XS("90 90 90 90 90 90")), // skybox fix
		//BytePatch("server.dll", "75 ? 44 38 A7 ? ? ? ? 75 ? 41 3B DD", 0x0, "EB"), // listen server speedhack
		BytePatch(XS("vguimatsurface.dll"), XS("66 83 FE ? 0F 84"), 0x0, XS("66 83 FE 00")), // include '&' in text size
	};
